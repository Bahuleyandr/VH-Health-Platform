 ...
 *   4. Payroll page loads and its 5 tab buttons are visible
 *   5. System logs loads without console errors
 */
...
test.describe('authenticated — admin dashboard journeys', () => {
  ...
  test('appointments list is reachable', async ({ page }) => {
    ...
    const tabs = ['Slot Availability', 'All Appointments', 'Doctor Queue', 'Prescriptions', 'Audit Trail'];
    let foundTabs = 0;
  ...
  test('payroll page renders with its 5 tab buttons', async ({ page }) => {
    ...
    await expect(page.getByRole('button', { name: /compliance/i })).toBeVisible({ timeout: 10_000 });
  });

  test('system logs page loads without client console errors', async ({ page }) => {
    const errors: string[] = [];

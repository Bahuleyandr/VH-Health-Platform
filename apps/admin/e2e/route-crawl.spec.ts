...
function discoverDashboardRoutes(): string[] {
  return [...new Set(walk(APP_ROOT).map(routeForPage).filter(Boolean) as string[])]
...
test.describe('authenticated — dashboard route crawl', () => {
  test('every static dashboard route renders without backend/proxy errors', async ({ context }) => {
    test.setTimeout(routeCrawlTimeoutMs);

    expect(routes.length, 'dashboard route discovery returned no routes').toBeGreaterThan(0);
    ...
    for (const route of routes) {
      await test.step(route, async () => {
          ...
          if (isBackendSurface && response.status() >= 400) {
            failedResponseReads.push((async () => {
              ...
              failedResponses.push(
                `${response.request().method()} ${response.status()} ${url}${bodySnippet ? ` :: ${bodySnippet}` : ''}`,
              );
            })());
          }
        });
        ...
        try {
          ...
          await expect(failureText.first(), `${route} should not render a visible error`).toHaveCount(0);

...
jest.mock(
  "@/app/(with-auth)/dashboard/dialysis/components/TodayBoardTab",
  () => ({
    __esModule: true,
...
describe("<DialysisPage />", () => {
  ...
  it('subscribes to staff:dialysis-board on the ["dialysis"] root and shows ○ Polling when down', () => {
    ...
    expect(mockRealtime).toHaveBeenCalledWith("staff:dialysis-board", [
      ["dialysis"],
    ]);
  });
  ...
  it("shows ● Live when subscribed", () => {
    ...
    renderPage(<DialysisPage />);
    expect(screen.getByTestId("dialysis-realtime-indicator")).toHaveTextContent(
      "Live",
    );
  });
});

...
const staffCards: HomeCard[] = [
  { label: "Today's Appointments", href: "/dashboard/my-appointments", icon: CalendarCheck, desc: "Patient queue" },
  { label: "Attendance", href: "/dashboard/my-attendance", icon: Activity, desc: "Monthly register" },
  { label: "Leave", href: "/dashboard/my-leave", icon: Inbox, desc: "Balance and requests" },
  { label: "Payslips", href: "/dashboard/my-payslips", icon: FileText, desc: "Earnings records" },
  { label: "Replacements", href: "/dashboard/my-replacements", icon: RefreshCw, desc: "Shift coverage" },
  { label: "Upload Documents", href: "/dashboard/upload-prescription", icon: Upload, desc: "Prescriptions and reports" },
];

const doctorCards: HomeCard[] = [
  { label: "Today's Patient Queue", href: "/dashboard/my-appointments", icon: Stethoscope, desc: "Scheduled appointments" },
  { label: "Pending Confirmations", href: "/dashboard/my-appointments?tab=pending", icon: AlertTriangle, desc: "Awaiting confirmation" },
  { label: "Upload Prescription", href: "/dashboard/upload-prescription", icon: ClipboardList, desc: "Post-consultation docs" },
  { label: "My Attendance", href: "/dashboard/my-attendance", icon: Activity, desc: "Attendance calendar" },
  { label: "My Leave", href: "/dashboard/my-leave", icon: Inbox, desc: "Leave balance" },
  { label: "My Payslips", href: "/dashboard/my-payslips", icon: FileText, desc: "Earnings overview" },
];

const hrCards: HomeCard[] = [
  { label: "Staff Roster", href: "/dashboard/staff-roster", icon: Users, desc: "Directory, onboarding, shifts", primary: true },
  { label: "Leave Approvals", href: "/dashboard/leave-approvals", icon: CalendarCheck, desc: "Pending leave requests" },
  { label: "Attendance Audit", href: "/dashboard/attendance-audit", icon: BarChart3, desc: "Anomalies and reports" },
  { label: "Open Incidents", href: "/dashboard/incidents", icon: AlertTriangle, desc: "Incident reports" },
  { label: "Grievances", href: "/dashboard/grievances", icon: Inbox, desc: "Confidential cases" },
  { label: "Report Audit", href: "/dashboard/reporting", icon: ShieldCheck, desc: "Compliance reports" },
];
    ...
    async function loadHRHome() {
      try {
        const [dashboardData, staffData] = await Promise.all([
          ...
          getStaffList<StaffListResponse>({ limit: 200, active: true }).catch(() => null),
        ]);
        if (!alive) return;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
        {[
          { label: "Active Staff", value: overview?.active_staff ?? staffList.length, icon: UserRoundCheck },
          { label: "Departments", value: departments.length, icon: IdCard },
          { label: "Checked In", value: overview?.currently_checked_in ?? 0, icon: Activity },
          { label: "Pending Leave", value: dashboard?.leaves?.pending ?? 0, icon: CalendarCheck },
        ].map((stat) => {
          ...
          return (
            <div key={stat.label} className="rounded-lg border border-border bg-card p-4">
              ...
              <p className="mt-2 text-2xl font-semibold text-white">{loading ? "--" : stat.value}</p>
            </div>
          );
        })}
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-[1.4fr_1fr]">
              ...
              <tbody>
                {seededStaff.length === 0 ? (
                  ...
                  seededStaff.map((staff) => (
                    <tr key={staff.employee_id ?? staff.uid ?? staff.id} className="border-t border-border">
                      <td className="px-4 py-3 font-medium text-white">{staff.employee_id}</td>
        ...
        <section className="rounded-lg border border-border bg-card">
          <div className="border-b border-border px-4 py-3">
            <h2 className="text-base font-semibold text-white">Department Coverage</h2>
          </div>
          <div className="divide-y divide-border">
            {(departments.length ? departments.slice(0, 6) : []).map((dept) => (
              <div key={dept.department} className="flex items-center justify-between gap-3 px-4 py-3 text-sm">
                <div>
                  <p className="font-medium text-white">{dept.department}</p>
                  <p className="text-xs text-muted-foreground">{dept.present_today} present today</p>
                </div>
            ...
            {!departments.length && (
              <div className="px-4 py-6 text-sm text-muted-foreground">No department coverage loaded</div>
            )}
          </div>
        </section>
      </div>

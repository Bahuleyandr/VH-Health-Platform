...
import {
  Calendar, Clock, Users, AlertTriangle, MapPin,
...
interface GeofenceData {
  ...
  frequent_offenders: Array<{ name: string; department: string; breach_count: number; last_breach: string }>;
}
...
export default function AttendanceAuditPage() {
  ...
  const dashQuery = useQuery<AttendanceDashboardData>({
    ...
    queryFn: async () => {
      const r = await getJSON<unknown>('/api/v1/staff/admin/audit/attendance/dashboard');
      return ((r as { data?: AttendanceDashboardData }).data ?? r) as AttendanceDashboardData;
    },
  ...
  const activityQuery = useQuery<HRActivityData>({
    ...
    queryFn: async () => {
      const r = await getJSON<unknown>(`/api/v1/staff/admin/audit/attendance/hr-activity?days=${days}`);
  ...
  const slaQuery = useQuery<SLAData>({
    ...
    queryFn: async () => {
      const r = await getJSON<unknown>(`/api/v1/staff/admin/audit/attendance/sla?days=${days}`);
  ...
  const geofenceQuery = useQuery<GeofenceData>({
    ...
    queryFn: async () => {
      const r = await getJSON<unknown>('/api/v1/staff/admin/audit/attendance/geofence');
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Super-admin oversight of leave, regularization, disputes, overtime, and HR actions
          </p>
        </div>
        <div className="flex items-center gap-2">
        ...
        <div className="space-y-6">
          ...
          {!dashQuery.isLoading && (
            <div className="bg-white border border-gray-200 rounded-xl p-4 flex items-center gap-4">
              ...
              <div className="flex-1">
                <p className="text-sm font-semibold text-gray-700">Geofence Breaches (last 7 days)</p>
                <p className="text-xs text-gray-500">
                  {String(dash?.geofence?.this_week ?? 0)} breaches across {String(dash?.geofence?.unique_staff ?? 0)} staff members
                </p>
              </div>
              <button
          ...
          <section>
            <h2 className="text-sm font-semibold text-gray-500 uppercase tracking-wide mb-3 flex items-center gap-2">
              ...
              {totalOverdue > 0 && (
                <span className="bg-red-100 text-red-700 text-xs px-2 py-0.5 rounded-full">{totalOverdue}</span>
              )}
            </h2>
                  ...
                  <table className="w-full text-sm">
                    <thead className="bg-gray-50 border-b">
                      <tr>
                        ...
                        <th className="text-left px-4 py-2.5 text-xs text-gray-500 font-medium">Subject</th>
                        <th className="text-left px-4 py-2.5 text-xs text-gray-500 font-medium">Submitted</th>
                        <th className="text-left px-4 py-2.5 text-xs text-gray-500 font-medium">Time Overdue</th>
                      </tr>
                    </thead>
                      ...
                      {dash?.overdue_items?.map((item, i) => (
                        <tr key={i} className="hover:bg-gray-50">
                          <td className="px-4 py-3">
                            <span className={`text-xs px-2 py-0.5 rounded-full font-medium capitalize ${typeColor(item.type)}`}>
                              {item.type}
                            </span>
                          </td>
                          <td className="px-4 py-3 text-gray-700 truncate max-w-64">{item.subject?.replace(/_/g, ' ')}</td>
                          <td className="px-4 py-3 text-gray-500 text-xs">{hoursAgo(item.created_at)}</td>
        ...
        <div className="space-y-6">
          {activityQuery.isLoading ? <Skeleton className="h-64 rounded-xl" /> : (
            <>
              ...
              <section>
                <h2 className="text-sm font-semibold text-gray-500 uppercase tracking-wide mb-3 flex items-center gap-2">
                  <Users size={14} /> HR / Admin Action Breakdown (last {days} days)
                </h2>
                {(activityQuery.data?.actors?.length ?? 0) === 0 ? (
                  <div className="bg-gray-50 rounded-xl p-6 text-center text-sm text-gray-500">No HR activity recorded</div>
                ) : (
                    ...
                    <table className="w-full text-sm min-w-max">
                      <thead className="bg-gray-50 border-b">
                        <tr>
                          ...
                          <th className="text-right px-4 py-2.5 text-xs text-blue-500 font-medium">Leave</th>
                          <th className="text-right px-4 py-2.5 text-xs text-orange-500 font-medium">Regulariz.</th>
                          <th className="text-right px-4 py-2.5 text-xs text-red-500 font-medium">Disputes</th>
                          <th className="text-right px-4 py-2.5 text-xs text-purple-500 font-medium">Overtime</th>
                      ...
                      <tbody className="divide-y">
                        {activityQuery.data?.actors?.map(a => {
                          const total = a.leave + a.regularization + a.disputes + a.overtime;
                          return (
                            <tr key={a.id} className="hover:bg-gray-50">
                              <td className="px-4 py-3 font-medium">{a.name}</td>
                              <td className="px-4 py-3 text-gray-500 text-xs">{a.role}</td>
                              <td className="px-4 py-3 text-right">{a.leave || '—'}</td>
                              <td className="px-4 py-3 text-right">{a.regularization || '—'}</td>
                              <td className="px-4 py-3 text-right">{a.disputes || '—'}</td>
                              <td className="px-4 py-3 text-right">{a.overtime || '—'}</td>
              ...
              {(activityQuery.data?.bulk_corrections?.length ?? 0) > 0 && (
                <section>
                  ...
                  <div className="bg-white border border-amber-200 rounded-xl divide-y text-sm">
                    {activityQuery.data?.bulk_corrections?.map((c, i) => (
                      <div key={i} className="flex items-center gap-3 px-4 py-3">
                        <div className="flex-1">
                          <p className="font-medium">{String(c.staff_name ?? '—')}</p>
                          <p className="text-xs text-gray-500">{String(c.date ?? '')} · {String(c.reason ?? '')}</p>
                        </div>
                        <div className="text-right text-xs">
                          <p className="text-gray-600">{String(c.reviewed_by_name ?? 'Unreviewed')}</p>
                          <p className={c.status === 'approved' ? 'text-green-600 font-medium' : 'text-gray-400'}>{String(c.status ?? '')}</p>
                        </div>
                      </div>
                    ))}
                  </div>
        ...
        <div className="space-y-4">
          {slaQuery.isLoading ? <Skeleton className="h-64 rounded-xl" /> : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {(['leave', 'regularization', 'disputes', 'overtime'] as const).map(key => {
                const row = slaQuery.data?.[key];
                if (!row) return null;
                const total = row.total || 0;
                const actioned = row.actioned || 0;
                const withinSLA = row.within_sla || 0;
                const compliancePct = actioned > 0 ? Math.round((withinSLA / actioned) * 100) : null;
                const overdueCount = row.overdue || 0;

                return (
                  <div key={key} className={`bg-white border rounded-xl p-5 ${overdueCount > 0 ? 'border-red-300' : 'border-gray-200'}`}>
                    ...
                    <div className="grid grid-cols-3 gap-2 text-center mb-3">
                      ...
                      <div>
                        <p className="text-2xl font-bold text-green-600">{actioned}</p>
                        <p className="text-xs text-gray-500">Actioned</p>
                      </div>
                      <div>
                        <p className={`text-2xl font-bold ${compliancePct !== null && compliancePct < 70 ? 'text-red-600' : 'text-green-600'}`}>
      ...
      {tab === 'geofence' && (
        <div className="space-y-6">
          {geofenceQuery.isLoading ? <Skeleton className="h-64 rounded-xl" /> : (
            <>
              {/* Stats row */}
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <StatCard label="Total Breaches" value={String(geofenceQuery.data?.stats?.total ?? 0)} />
                <StatCard label="This Week" value={String(geofenceQuery.data?.stats?.this_week ?? 0)} alert={(geofenceQuery.data?.stats?.this_week ?? 0) > 5} />
                <StatCard label="Unique Staff" value={String(geofenceQuery.data?.stats?.unique_staff ?? 0)} />
                <StatCard label="Check-in Outside" value={String(geofenceQuery.data?.stats?.checkin_outside ?? 0)} alert={(geofenceQuery.data?.stats?.checkin_outside ?? 0) > 0} />
              </div>
                  ...
                  <div className="bg-white border border-gray-200 rounded-xl divide-y">
                    {geofenceQuery.data?.frequent_offenders?.map((o, i) => (
                      <div key={i} className="flex items-center gap-3 px-4 py-3">
                        ...
                        <p className="text-xs text-gray-400">Last: {hoursAgo(o.last_breach)}</p>
                      </div>
                    ))}
                  </div>
                </section>
                    ...
                    <div className="divide-y max-h-96 overflow-y-auto">
                      {geofenceQuery.data?.breaches?.map(b => (
                        <div key={b.id} className="flex items-center gap-3 px-4 py-3">
                          <MapPin size={14} className={b.action === 'checkin_outside' ? 'text-red-500' : 'text-orange-400'} />
                          <div className="flex-1 min-w-0">
                            <p className="text-sm font-medium truncate">{b.staff_name}</p>
                            <p className="text-xs text-gray-500">{b.department} · {b.action.replace(/_/g, ' ')}
                              {b.distance_meters ? ` · ${b.distance_meters}m from campus` : ''}
                            </p>
                          </div>
                          <p className="text-xs text-gray-400 shrink-0">{hoursAgo(b.occurred_at)}</p>
                          {!b.alerted && (
                            <span className="text-xs bg-yellow-50 text-yellow-700 border border-yellow-200 px-1.5 py-0.5 rounded shrink-0">unalerted</span>
                          )}
                        </div>
                      ))}
                    </div>
...
// ─── Sub-components ───────────────────────────────────────────────────────────
function ModuleCard({
  icon, label, pending, overdue, approved, rejected, slaLabel
}: {
  icon: React.ReactNode; label: string;
  pending: number; overdue: number; approved: number; rejected: number; slaLabel: string;
}) {
  return (
    <div className={`bg-white border rounded-xl p-4 ${overdue > 0 ? 'border-red-300' : 'border-gray-200'}`}>
      <div className="flex items-center gap-2 mb-2">{icon}<span className="text-xs text-gray-500 font-medium">{label}</span></div>
      <p className={`text-3xl font-bold mb-1 ${overdue > 0 ? 'text-red-600' : 'text-gray-900'}`}>{pending}</p>
      <p className="text-xs text-gray-500">pending</p>
      {overdue > 0 && <p className="text-xs text-red-600 font-medium mt-1">{overdue} past {slaLabel}</p>}

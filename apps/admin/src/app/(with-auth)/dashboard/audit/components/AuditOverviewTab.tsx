...
import { AlertTriangle, Clock, Users, TrendingUp, Activity, CheckCircle, XCircle, Eye } from "lucide-react";
...
export function AuditOverviewTab({ data: dash, isLoading, onOpenTrail }: Props) {
  return (
                ...
                <tbody className="divide-y">
                  {dash?.sla_breaches?.map(b => (
                    <tr key={`${b.type}-${b.id}`} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3">
                        <button
                          onClick={() => onOpenTrail({ type: b.type as "incident" | "grievance", id: String(b.id), number: b.report_number })}

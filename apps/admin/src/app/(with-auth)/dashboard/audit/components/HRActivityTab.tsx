        ...
        <>
          ...
          <div className="grid grid-cols-2 gap-4">
            {data?.resolution_stats?.map((rs) => (
              ...
              >
                <p className="text-xs text-gray-500 uppercase font-medium mb-1 capitalize">
                  {rs.type} Resolution
                </p>
                <p className="text-3xl font-bold text-gray-900">
                  {rs.resolution_rate_pct ?? 0}%
                </p>
                <p className="text-sm text-gray-500">
                  {rs.resolved}/{rs.total} resolved · avg{" "}
                  {formatHours(rs.avg_hours_to_resolve)}
                </p>
              </div>
            ))}
          </div>
          ...
          <section>
            <h2 className="text-sm font-semibold text-gray-500 uppercase tracking-wide mb-3">
              HR / Admin Actions (last {days} days)
            </h2>
          ...
          {(data?.neglected_reports?.length ?? 0) > 0 && (
            <section>
              ...
              <div className="bg-card border border-red-200 rounded-xl divide-y">
                {data?.neglected_reports?.map((r, i) => (
                  <div key={i} className="flex items-center gap-3 px-4 py-3">
                    <span
                      className={`text-xs px-2 py-0.5 rounded-full font-bold ${r.type === "incident" ? "bg-orange-100 text-orange-700" : "bg-purple-100 text-purple-700"}`}
                    >
                      {r.report_number}
                    </span>
                    <p className="flex-1 text-sm truncate">{r.subject}</p>
                    <span className="text-xs text-red-600 font-medium">
                      {formatHours(r.hours_open)} open
                    </span>

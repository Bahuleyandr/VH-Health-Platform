...
export function PerformanceTab() {
  ...
  const sortedFlagStaff = Object.entries(flagByStaff).sort(
    (a, b) => b[1] - a[1],
  );
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-lg font-semibold text-gray-800">
          Staff Performance Overview (30 days)
        </h2>
        <button
          onClick={() => refetch()}
                ...
                {top_staff.map((s, i) => (
                  <tr key={s.id} className="border-t hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-gray-500">
                      {s.avg_minutes
                        ? `${Math.round(parseInt(s.avg_minutes))} min`
                        : "—"}
                    </td>
                  </tr>
                ))}
      ...
      {recent_flags.length > 0 && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-card rounded-xl border border-red-100 p-5">
              ...
              <tbody>
                {recent_flags.map((f) => (
                  <tr key={f.id} className="border-t hover:bg-red-50">
                    <td className="px-4 py-2 font-mono text-xs text-red-700">
                      {f.log_number}
                    </td>
                    ...
                    <td
                      className="px-4 py-2 text-red-600 text-xs max-w-[200px] truncate"
                      title={f.flag_reason ?? ""}
                    >
                      {f.flag_reason ?? "—"}
                    </td>

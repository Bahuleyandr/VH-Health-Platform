  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [queueRes, pendingRes] = await Promise.allSettled([
        fetch(
          `${API_BASE_URL}${API_ENDPOINTS.myWork.appointments.todayQueue}`,
          { headers },
        ),
        fetch(`${API_BASE_URL}${API_ENDPOINTS.myWork.appointments.pending}`, {
          headers,
        }),
      ]);

      if (queueRes.status === "fulfilled" && queueRes.value.ok) {
        const d = (await queueRes.value.json()) as
          | { data?: Appointment[]; appointments?: Appointment[] }
          | Appointment[];
        const items = Array.isArray(d)
          ? d
          : ((d as { data?: Appointment[]; appointments?: Appointment[] })
              .data ??
            (d as { appointments?: Appointment[] }).appointments ??
            []);
        setQueue(items);
      }
      if (pendingRes.status === "fulfilled" && pendingRes.value.ok) {
        const d = (await pendingRes.value.json()) as
          | { data?: Appointment[]; appointments?: Appointment[] }
          | Appointment[];
        const items = Array.isArray(d)
          ? d
        ...
        list.map((appt) => (
          ...
          >
            <div className="flex items-start justify-between">
              ...
              <span
                className={`rounded-full px-2.5 py-0.5 text-xs font-medium ${statusBadge(appt.status)}`}
              >
                {appt.status}
              </span>
            </div>
            ...
            <div className="flex gap-2 flex-wrap">
              ...
              {appt.status === "COMPLETED" && (
                <button
                  onClick={() =>
                    router.push(
                      `/dashboard/upload-prescription?appointmentId=${appt.id}`,
                    )
                  }

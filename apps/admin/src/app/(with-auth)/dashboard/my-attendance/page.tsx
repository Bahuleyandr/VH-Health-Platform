...
export default function MyAttendancePage() {
  ...
  const fetchAttendance = useCallback(async () => {
    ...
    try {
      ...
      const d = (await res.json()) as
        | { data?: AttendanceRecord[]; records?: AttendanceRecord[] }
        | AttendanceRecord[];
      const items = Array.isArray(d)
        ? d
        : ((d as { data?: AttendanceRecord[] }).data ??
          (d as { records?: AttendanceRecord[] }).records ??
          []);
      setRecords(items);
    } catch (e) {
  ...
  const handleRegularize = async () => {
    ...
    try {
      await fetch(
        `${API_BASE_URL}${API_ENDPOINTS.myWork.attendance.regularize}`,
        {
          method: "POST",
          headers,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        <button
          onClick={prevMonth}
          className="rounded bg-muted px-3 py-1.5 text-sm text-muted-foreground hover:text-foreground"
        >
          ‹
        </button>
            ...
            {Array.from({ length: daysInMonth }).map((_, i) => {
              ...
              return (
                <div
                  ...
                  title={
                    rec
                      ? `${rec.status}${rec.check_in ? ` · In: ${rec.check_in}` : ""}${rec.check_out ? ` · Out: ${rec.check_out}` : ""}`
                      : dateStr
                  }
                  className={`
          ...
          <div className="rounded-xl border border-border bg-card p-6 w-full max-w-md space-y-4">
            ...
            <div className="flex gap-2 justify-end">
              ...
              <button
                ...
                disabled={submitting || !formDate || !formReason}

...
export default function MyPayslipsPage() {
  ...
  const fetchPayslips = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `${CLIENT_API_BASE_URL}${API_ENDPOINTS.myWork.payslips.list}`,
        { headers },
      );
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      const d = (await res.json()) as
        | { data?: Payslip[]; payslips?: Payslip[] }
        | Payslip[];
      const items = Array.isArray(d)
        ? d
        : ((d as { data?: Payslip[] }).data ??
          (d as { payslips?: Payslip[] }).payslips ??
          []);
      setPayslips(items);
    } catch (e) {
  ...
  const downloadPayslip = async (id: string) => {
    ...
    try {
      const res = await fetch(
        `${CLIENT_API_BASE_URL}${API_ENDPOINTS.myWork.payslips.download(id)}`,
        { headers },
      );
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  Gross: {formatCurrency(p.gross_pay)} · Deductions:{" "}

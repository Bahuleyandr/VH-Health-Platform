...
export default function MyReplacementsPage() {
  ...
  const fetchRequests = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `${API_BASE_URL}${API_ENDPOINTS.myWork.replacements.list}`,
        { headers },
      );
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      const d = (await res.json()) as
        | { data?: ReplacementRequest[]; requests?: ReplacementRequest[] }
        | ReplacementRequest[];
      const items = Array.isArray(d)
        ? d
        : ((d as { data?: ReplacementRequest[] }).data ??
          (d as { requests?: ReplacementRequest[] }).requests ??
          []);
      setRequests(items);
    } catch (e) {
  ...
  const createRequest = async () => {
    ...
    try {
      const res = await fetch(
        `${API_BASE_URL}${API_ENDPOINTS.myWork.replacements.create}`,
        {
          method: "POST",
          headers,
            ...
            <div className="flex items-start justify-between">
              <div>
                <p className="font-semibold text-white">
                  {req.date} · {req.shift} Shift
                </p>
                {req.requested_staff && (
                  <p className="text-sm text-muted-foreground">
                    Requested replacement: {req.requested_staff}
                  </p>
                )}
              ...
              <span
                className={`rounded-full px-2.5 py-0.5 text-xs font-medium ${STATUS_STYLE[req.status] ?? "bg-gray-500/20 text-gray-400"}`}
              >
                {req.status}
              </span>
            </div>
            ...
            <div className="space-y-3">
              ...
              <label className="block text-sm text-muted-foreground">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setForm((f) => ({ ...f, requested_staff: e.target.value }))
                  }

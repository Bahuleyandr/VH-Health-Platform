                  ...
                  {detail.payslips.map((p) => (
                    <tr key={p.id} className="hover:bg-gray-50">
                      <td className="px-3 py-2 font-medium">
                        {p.staff_name ?? p.staff_uid?.slice(0, 8)}
                      </td>
                      <td className="px-3 py-2 text-gray-500">
                        {p.department ?? "—"}
                      </td>
                      <td className="px-3 py-2">
                        {p.days_present}/{p.total_working_days}
                      </td>
                      <td className="px-3 py-2">
                        {fmtCurrency(p.gross_salary)}
                      </td>
                      <td className="px-3 py-2 text-red-600">
                        {fmtCurrency(p.total_deductions)}
                      </td>
                      <td className="px-3 py-2 font-bold text-teal-700">
                        {fmtCurrency(p.net_salary)}
                      </td>
      ...
      >
        <div className="space-y-4">
          <p className="text-sm text-gray-600">
            This will calculate payslips for all staff with salary config based
            on attendance and leave data.
          </p>
          ...
          >
            {runMut.isPending
              ? "Processing..."
              : `Run Payroll for ${fmtMonth(runMonth)} ${runYear}`}
          </button>
        </div>
      </Modal>
    </div>

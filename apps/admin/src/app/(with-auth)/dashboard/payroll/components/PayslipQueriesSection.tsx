            ...
            >
              <div className="flex-1">
                ...
                <div className="text-xs text-gray-400 mt-0.5">
                  {q.staff_name} · {MONTHS[(q.month - 1 + 12) % 12]} {q.year} ·{" "}
                ...
                {q.replies && q.replies.length > 0 && (
                  <div className="space-y-2 mb-3">
                    {q.replies.map((r) => (
                      <div
                        key={r.id}
                        className={`flex gap-2 text-xs ${r.author_role === "HR" || r.author_role === "ADMIN" ? "flex-row-reverse" : ""}`}
                      >
                        <div
                          className={`rounded-lg px-3 py-2 max-w-sm ${r.author_role === "HR" || r.author_role === "ADMIN" ? "bg-teal-50 text-teal-900" : "bg-card border text-gray-700"}`}
                        >
                ...
                {q.status !== "resolved" && (
                  <div className="flex gap-2 items-end">
                    <textarea
                      ...
                      onChange={(e) =>
                        setReplyText((r) => ({ ...r, [q.id]: e.target.value }))
                      }
                    />
                    <div className="flex flex-col gap-1">
                      <label className="flex items-center gap-1 text-xs text-gray-600">
                        <input
                          type="checkbox"
                          checked={resolveCheck[q.id] ?? false}
                          onChange={(e) =>
                            setResolveCheck((r) => ({
                              ...r,
                      ...
                      <button
                        onClick={() =>
                          replyMut.mutate({
                            id: q.id,
                            message: replyText[q.id] ?? "",
                            resolve: resolveCheck[q.id] ?? false,
                          })
                        }

    ...
    <div className="rounded border bg-white p-4">
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <div className="space-y-3">
          <TextField label="Facility name" value={form.name} onChange={set("name")} placeholder="e.g. Apollo Speciality Hospital" />
          <label className="block">
            <span className="mb-1 block text-xs font-semibold uppercase tracking-wide text-slate-500">Facility type</span>
            <select
              value={form.facilityType}
              onChange={(event) =>
                setForm((prev) => ({ ...prev, facilityType: event.target.value as ReferralFacilityType }))
              }
          ...
          <TextField label="Phone" value={form.phone} onChange={set("phone")} placeholder="+91 44 2829 3333" />
          <TextField label="Email" value={form.email} onChange={set("email")} placeholder="referrals@facility.example" />
        </div>
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <TextField label="PIN code" value={form.pincode} onChange={set("pincode")} placeholder="600006" />
          </div>
          <label className="block">
...
export default function ReferralFacilitiesPage() {
  ...
  const createMutation = useMutation({
    mutationFn: (payload: ReferralFacilityWrite) => createReferralFacility(payload),
    onSuccess: () => {
    ...
    onError: (err: unknown) =>
      toast.error(err instanceof Error ? err.message : "Could not create facility"),
  });

  const updateMutation = useMutation({
    mutationFn: ({ id, payload }: { id: number; payload: ReferralFacilityWrite }) =>
    ...
    onError: (err: unknown) =>
      toast.error(err instanceof Error ? err.message : "Could not update facility"),
  });
  ...
  const activeMutation = useMutation({
    ...
    onSuccess: (facility) => {
      toast.success(facility.active ? "Facility reactivated" : "Facility deactivated");
      invalidate();
    },
    onError: (err: unknown) =>
      toast.error(err instanceof Error ? err.message : "Could not change facility status"),
  });
    ...
    <div className="space-y-4 p-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 max-w-3xl text-sm text-muted-foreground">
            Master list of external destination facilities. External referrals link a facility
            from this list so outbound care is trackable and reportable; deactivating a facility
            stops new referrals without touching historical ones.
          </p>
        </div>
        <button
          type="button"
      ...
      {editing && (
        <FacilityForm
          ...
          onSubmit={() => updateMutation.mutate({ id: editing.id, payload: toPayload(form) })}
        />
      )}
      ...
      {facilitiesQuery.isLoading ? (
        ...
        <div className="rounded border bg-white p-10 text-center text-sm text-muted-foreground">
          No destination facilities yet. Add the hospitals and centres you refer patients to.
        </div>
      ) : (
          ...
          <table className="w-full min-w-[960px] text-left text-sm">
            ...
            <tbody>
              {facilities.map((facility) => (
                <tr key={facility.id} className="border-b last:border-0 hover:bg-slate-50/60">
                  <td className="px-4 py-3">
                    <div className="font-medium">{facility.name}</div>
                    {facility.notes && (
                      <div className="max-w-72 truncate text-xs text-muted-foreground">{facility.notes}</div>
                    )}
                  </td>
                  <td className="px-4 py-3">{TYPE_LABELS[facility.facilityType] ?? facility.facilityType}</td>
                  <td className="px-4 py-3">
                    {facility.specialties.length === 0 ? (
                      ...
                      <div className="flex max-w-64 flex-wrap gap-1">
                        {facility.specialties.map((tag) => (
                          <span key={tag} className="rounded-full bg-slate-100 px-2 py-0.5 text-xs">{tag}</span>
                        ))}
                      </div>
                    )}
                  </td>
                  <td className="px-4 py-3">
                    <div>{[facility.city, facility.state].filter(Boolean).join(", ") || "—"}</div>
                    {facility.pincode && <div className="text-xs text-muted-foreground">{facility.pincode}</div>}
                  </td>
                  <td className="px-4 py-3">
                  ...
                  <td className="px-4 py-3">
                    <span
                      className={`rounded-full px-2.5 py-0.5 text-xs font-semibold ${
                        facility.active ? "bg-emerald-100 text-emerald-800" : "bg-slate-200 text-slate-600"
                      }`}
                    >
                  ...
                  <td className="px-4 py-3">
                    <div className="flex justify-end gap-1">
                      ...
                      <button
                        type="button"
                        onClick={() => activeMutation.mutate({ id: facility.id, active: !facility.active })}
                        disabled={activeMutation.isPending}
                        className={`rounded p-2 hover:bg-slate-100 disabled:opacity-50 ${
                          facility.active ? "text-red-500" : "text-emerald-600"
                        }`}
                        aria-label={facility.active ? `Deactivate ${facility.name}` : `Reactivate ${facility.name}`}

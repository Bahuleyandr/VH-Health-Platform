...
function TodayTab() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const r = await fetchAdminAPI<{ data: TheatreSchedule[] }>(
        "/theatre/today",
      );
      ...
      setError(
        e instanceof Error ? e.message : "Failed to load theatre schedule",
      );
    } finally {
      setLoading(false);
    }
  ...
  const updateStatus = async (id: number, status: string) => {
    setUpdating(id);
    try {
      await putJSON(`/api/v1/theatre/${id}/status`, { status });
      load();
    } catch (e) {
  ...
  const cancel = async (id: number) => {
    ...
    try {
      await deleteJSON(`/api/v1/theatre/${id}`);
      load();
    } catch (e) {
                ...
                >
                  ...
                  <td className="py-2 px-3 flex gap-2">
                    <select
                      ...
                      onChange={(e) => {
                        if (e.target.value) updateStatus(s.id, e.target.value);
                      }}
...
function NewBookingTab() {
  ...
  const submit = async () => {
    if (!form.patient_uid || !form.procedure_name || !form.surgeon_uid) {
      alert("Patient UID, procedure name, and surgeon UID are required");
      return;
    }
    setSaving(true);
    setSuccess(false);
...
function AvailabilityTab() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const r = await fetchAdminAPI<{ data: TheatreAvailability[] }>(
  ...
  return (
    <div className="space-y-4">
      ...
      {availability.length > 0 &&
        availability.map((a) => (
          <div
            key={a.theatre_number}
            className="border border-border rounded-lg p-4"
          >
            <h3 className="font-semibold mb-2">
              Theatre {a.theatre_number} — {a.date}
            </h3>
    ...
    <div className="p-6">
      ...
      <div className="flex gap-1 bg-muted rounded-lg p-1 mb-6">
        {[
          ...
          { key: "new" as const, label: "+ New Booking" },
...
export default function TheatrePage() {
  return (
    <Suspense fallback={<div className="p-6">Loading theatre...</div>}>
      <TheatreContent />
    </Suspense>
  );
}

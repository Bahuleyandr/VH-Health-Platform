...
function extractToken(data: unknown): string | undefined {
  ...
  if (hasKey(data, "token") && typeof data.token === "string")
    return data.token;
  if (
    hasKey(data, "data") &&
    isRecord(data.data) &&
...
(async () => {
  ...
  logResult(
    "Auth: /admin/auth/me",
    await call("/admin/auth/me", "GET", undefined, token),
  );
  ...
  if (
    isRecord(settingsList.data) &&
    Array.isArray(settingsList.data.settings)
  ) {
    const first = settingsList.data.settings[0];
    if (isRecord(first) && typeof first.key === "string")
      firstSettingKey = first.key;
  }
  if (firstSettingKey) {
  ...
  logResult(
    "Users: GET /users",
    await call("/users?limit=5", "GET", undefined, token),
  );
  logResult(
    "Users: GET /users/inactive",
    await call("/users/inactive", "GET", undefined, token),
  );
  ...
  logResult(
    "Logs: GET /logs/audit",
    await call("/logs/audit?limit=5", "GET", undefined, token),
  );
  logResult(
    "Logs: GET /logs/system",
    await call("/logs/system?limit=5", "GET", undefined, token),
  );
  logResult(
    "Logs: Export audit",
    await call("/logs/audit/export?format=json", "GET", undefined, token),
  );
  logResult(
    "Logs: Export system",
    await call("/logs/system/export?format=json", "GET", undefined, token),
  );

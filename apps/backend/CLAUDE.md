...
- **★★ The migration runner OWNS two session GUCs; migrations must use `SET LOCAL`.** `ci-setup-db.mjs` applies the whole chain through ONE long-lived connection, so a plain `SET` outlives the file that issued it and governs every migration after it. `000_baseline.sql` is `pg_dump` output and its preamble issues nine of them — which is exactly how `check_function_bodies = false` leaked and let migrations 744 and 745 ship plpgsql bodies that cannot compile while CI stayed green. `scripts/lib/migrationSessionGucs.mjs` now pins `check_function_bodies = on` and `row_security = off` before the first migration and again after each one, and `assertMigrationSessionGucs()` fails the run if either drifted. `scripts/ci/check-migration-session-guc.mjs` blocks a NEW migration from re-introducing a bare `SET` of either (a `SET` inside a `CREATE FUNCTION` signature is a per-function attribute and is correctly ignored — migration 736 has three). **★★★ The two values pull opposite ways and that is deliberate: `row_security = off` does NOT mean "ignore RLS", it means "error instead of silently applying a policy". For a plain owner of a FORCE-RLS table, `off` raises 42501 while `on` returns ZERO ROWS SILENTLY — so a backfill would report success having touched nothing. Never "restore" `row_security` to `on`.** It is inert today (CI runs as superuser; the migration Job's `vhhealth` role is granted BYPASSRLS by `cluster.yaml`, gated by the `wait-owner-bypassrls` initContainer) — the pin exists so that a regenerated baseline, or a future withdrawal of BYPASSRLS, cannot silently turn loud failures into silent no-ops.

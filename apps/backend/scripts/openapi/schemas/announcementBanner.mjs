...
export const operations = {
  'GET /api/v1/notifications/announcement-banner': {
    description:
      'Returns the tenant-wide announcement banner shown across the admin portal dashboard, or a null banner when none is configured. Readable by any authenticated portal user.',
    response: 'AnnouncementBannerResponse',
  },
  'PUT /api/v1/notifications/announcement-banner': {
    description:
      'Saves the tenant-wide announcement banner (ADMIN or SUPER_ADMIN only). The text is sanitized and capped at 300 characters; saving with enabled=false or empty text hides the banner for all portal users. The change is audit-logged.',

...
// ---------------------------------------------------------------------------
// mar sub-domain enums (null-free; EXACT casing from the clinical/AI services
// reached via the /api/v1/emr/mar/* rewrite → clinicalRoutes.js). All
// app-layer-validated VARCHAR (no DB CHECK on these columns). The mar cluster
// is NOT the admissionRoutes alias — it lives under /api/v1/emr/mar only.
// NOTE: NEWS2_CLINICAL_RISK / AI_SAFETY_SEVERITY / CONSCIOUSNESS / looseObject /
// aiDraft / aiSafetyFlag / aiMetadata are already declared above — REUSED here.
// ---------------------------------------------------------------------------
// medication_administrations.status — service-enforced (marService); 'cancelled'
// seen in filters, 'refused' is stored via refusal_reason (no status write).
const MAR_STATUS = ['scheduled', 'held', 'administered', 'missed', 'cancelled'];
// medication_administrations.route — marService VALID_ROUTES (lowercase).
const MAR_ROUTE = [
  'oral', 'iv', 'im', 'sc', 'topical', 'inhaled', 'sublingual',
  'buccal', 'transdermal', 'rectal', 'intranasal', 'ophthalmic',
];
// five-rights context.drugMatchMode (marFiveRightsService) — nullable on row.
const MAR_DRUG_MATCH_MODE = ['name', 'pack_barcode'];
// drug-chart governance.state / safety.state (drugChartService).
const DRUG_CHART_GOV_STATE = ['blocked', 'fallback'];
const DRUG_CHART_GOV_OUTCOME = ['blocked', 'warning', 'clear'];
const DRUG_CHART_SAFETY_STATE = ['schema-unavailable', 'blocked', 'fallback'];
// medication_order pharmacy_status (drug-chart medication_orders[]).
const PHARMACY_STATUS = [
  'pending_indent', 'not_applicable', 'received_on_ward', 'issued_by_pharmacy',
  'approved', 'rejected', 'requested',
];
// news2_scores trend (news2Service.getPatientNEWS2History).
const NEWS2_TREND = ['stable', 'increasing', 'decreasing'];
// nurse_handovers.shift (handoverService).
const HANDOVER_SHIFT = ['morning', 'afternoon', 'night'];
// nurse_handovers.status (DB default 'pending'; flips to acknowledged).
const HANDOVER_STATUS = ['pending', 'acknowledged'];
// sttService provider (voice-note/config).
const STT_PROVIDER = ['none', 'local_whisper', 'azure', 'openai', 'mock'];
// clinical_voice_notes.transcript_status (service-set; DB default 'pending').
const TRANSCRIPT_STATUS = ['completed', 'failed', 'skipped', 'blocked', 'pending'];
// AI draft review_status for voice-soap / ambient (pending|failed only).
const AI_DRAFT_REVIEW_STATUS = ['pending', 'failed'];
// deteriorationEarlyWarningService band.
const DETERIORATION_BAND = ['stable', 'watch', 'concerning', 'critical'];
// polypharmacyAiService finding source.
const POLYPHARMACY_FINDING_SOURCE = ['rules', 'ai', 'defense'];
...
export const schemas = {
  ...
  // =========================================================================
  // mar sub-domain — MAR core (medication_administrations rows)
  // =========================================================================
  // MarRow — schedule (array item) + hold/miss superset row. LOOSE: the
  // idempotent-dup path returns a slightly different projection (adds
  // prescription_id), so additionalProperties:true. dose/dosage/route are
  // varchar (STRINGS). Reusable component for the 5 mutation/schedule shapes.
  MarRow: {
    type: 'object',
    additionalProperties: true,
    required: ['id', 'patient_uid', 'medication_name', 'status'],
    properties: {
      id: { type: 'integer' },
      patient_uid: { type: 'string', format: 'uuid' },
  ...
  // MarAdministeredRow — POST /mar/{id}/administer (no-scan). STRICT: fixed
  // RETURNING list; patient/medication_scanned_at null on the no-scan path.
  MarAdministeredRow: {
    type: 'object',
    additionalProperties: false,
    required: ['id', 'patient_uid', 'medication_name', 'status'],
  ...
  // MarScannedRow — POST /mar/{id}/administer-with-scan. STRICT + scan audit.
  // rights_passed is a jsonb OBJECT {patient,drug,dose,route,time} (booleans) —
  // typed as object, NEVER string.
  MarScannedRow: {
    type: 'object',
    additionalProperties: false,
    required: ['id', 'patient_uid', 'medication_name', 'status'],
    properties: {
      ...
      all_rights_passed: { type: 'boolean' },
      override_reason: { type: 'string', nullable: true },
      patient_scanned_at: { type: 'string', format: 'date-time', nullable: true },
      medication_scanned_at: { type: 'string', format: 'date-time', nullable: true },
    },
  ...
  // MarFiveRightsResult — POST /mar/verify dry-run {ma,rights,allPassed,context}.
  // STRICT and deterministic (no write). rights is a strict 5-boolean object.
  MarFiveRightsResult: {
    type: 'object',
    additionalProperties: false,
    ...
    properties: {
      ...
      context: {
        ...
        additionalProperties: false,
        properties: {
          minutesFromScheduled: { type: 'number', nullable: true },
          windowMinutes: { type: 'number' },
          drugMatchMode: { type: 'string', nullable: true, enum: MAR_DRUG_MATCH_MODE },
        },
      },
    },
  },

  // MarListRow — GET /mar/patient/{patientUid} reduced day-scoped projection
  // (NO dosage, NO tenant_id). Strict-ish: known projection but tolerate extras.
  MarListRow: {
    type: 'object',
    additionalProperties: true,
    required: ['id', 'patient_uid', 'medication_name', 'status'],
  ...
  // MarOverdueRow — GET /mar/overdue reduced. STRICT.
  MarOverdueRow: {
    type: 'object',
    additionalProperties: false,
    required: ['id', 'patient_uid', 'medication_name', 'status'],
  ...
  // MarDueRow — GET /mar/due + joined patient/bed/ward. Strict-ish.
  MarDueRow: {
    type: 'object',
    additionalProperties: true,
    required: ['id', 'patient_uid', 'medication_name', 'status'],
  ...
  // =========================================================================
  // mar sub-domain — Drug chart (composite detail)
  // =========================================================================
  // DrugChart — GET /drug-chart/admission/{id}. LOOSE composite with STRICT
  // permissions/governance sub-objects; admission/medication_orders/
  // administrations/pharmacy_indents kept loose (raw SELECT * / jsonb).
  DrugChart: {
    type: 'object',
    additionalProperties: true,
    required: ['admission', 'permissions', 'governance'],
    properties: {
      ...
      permissions: {
        ...
        additionalProperties: false,
        properties: {
          can_prescribe: { type: 'boolean' },
          can_administer: { type: 'boolean' },
          can_dispense: { type: 'boolean' },
        },
      },
      governance: {
        type: 'object',
        additionalProperties: true,
        properties: {
          state: { type: 'string', enum: DRUG_CHART_GOV_STATE },
          outcome: { type: 'string', enum: DRUG_CHART_GOV_OUTCOME },
          label: { type: 'string' },
      ...
      medication_orders: {
        ...
        items: {
          ...
          properties: {
            ...
            details: { type: 'object', additionalProperties: true },
            pharmacy_status: { type: 'string', nullable: true, enum: PHARMACY_STATUS },
            safety: {
              type: 'object',
              additionalProperties: true,
              properties: {
                state: { type: 'string', enum: DRUG_CHART_SAFETY_STATE },
                warnings: { type: 'array', items: { type: 'object', additionalProperties: true } },
                blockers: { type: 'array', items: { type: 'object', additionalProperties: true } },
              },
            },
            administrations: { type: 'array', items: { type: 'object', additionalProperties: true } },
          },
        },
      },
      administrations: { type: 'array', items: { type: 'object', additionalProperties: true } },
      pharmacy_indents: { type: 'array', items: { type: 'object', additionalProperties: true } },
    },
  },

  // =========================================================================
  // mar sub-domain — NEWS2
  // =========================================================================
  // News2Record — POST /mar/news2/record. STRICT row, but `data` can legitimately
  // be NULL (no usable vital parameter) → the envelope's data is nullable below.
  News2Record: {
    type: 'object',
    additionalProperties: true,
    required: ['id', 'patient_uid', 'total_score'],
    properties: {
      id: { type: 'integer' },
      patient_uid: { type: 'string', format: 'uuid' },
      total_score: { type: 'integer' },
  ...
  // News2HistoryRow — news2/patient scores[] full row. Vitals are numeric(→number).
  News2HistoryRow: {
    type: 'object',
    additionalProperties: true,
    required: ['id', 'patient_uid', 'total_score'],
    properties: {
      id: { type: 'integer' },
      patient_uid: { type: 'string', format: 'uuid' },
      respiration_rate: { type: 'number', nullable: true },
      spo2: { type: 'number', nullable: true },
      spo2_scale: { type: 'integer', nullable: true },
      supplemental_o2: { type: 'boolean', nullable: true },
      temperature: { type: 'number', nullable: true },
      systolic_bp: { type: 'number', nullable: true },
      heart_rate: { type: 'number', nullable: true },
      consciousness: { type: 'string', nullable: true },
      total_score: { type: 'integer' },
      clinical_risk: { type: 'string', enum: NEWS2_CLINICAL_RISK },
      escalation_action: { type: 'string', nullable: true },
      recorded_by: { type: 'string', format: 'uuid', nullable: true },
      recorded_at: { type: 'string', format: 'date-time', nullable: true },
    },
  },

  // News2History — wrapper {scores: News2HistoryRow[], trend}.
  News2History: {
    type: 'object',
    additionalProperties: true,
    required: ['scores', 'trend'],
    properties: {
      scores: { type: 'array', items: { $ref: '#/components/schemas/News2HistoryRow' } },
      trend: { type: 'string', enum: NEWS2_TREND },
    },
  },

  // =========================================================================
  // mar sub-domain — Handover (nurse_handovers; jsonb arrays)
  // =========================================================================
  // Handover — create/acknowledge row. jsonb array columns → typed array (NOT
  // string). acknowledge adds acknowledged_at + acknowledged=true (same schema).
  Handover: {
    type: 'object',
    additionalProperties: true,
    required: ['id', 'patient_uid', 'shift', 'status'],
    properties: {
      id: { type: 'integer' },
      tenant_id: { type: 'string', format: 'uuid' },
      patient_uid: { type: 'string', format: 'uuid' },
      ward: { type: 'string', nullable: true },
      bed_number: { type: 'string', nullable: true },
      outgoing_nurse: { type: 'string', nullable: true },
      incoming_nurse: { type: 'string', nullable: true },
      shift: { type: 'string', enum: HANDOVER_SHIFT },
      patient_summary: { type: 'string', nullable: true },
      summary: { type: 'string', nullable: true },
      active_issues: { type: 'array', items: { type: 'object', additionalProperties: true } },
      pending_tasks: { type: 'array', items: { type: 'object', additionalProperties: true } },
      medications_due: { type: 'array', items: { type: 'object', additionalProperties: true } },
      alerts: { type: 'array', items: { type: 'object', additionalProperties: true } },
      special_instructions: { type: 'string', nullable: true },
      status: { type: 'string', enum: HANDOVER_STATUS },
      acknowledged: { type: 'boolean' },
      acknowledged_at: { type: 'string', format: 'date-time', nullable: true },
      created_at: { type: 'string', format: 'date-time' },
      updated_at: { type: 'string', format: 'date-time' },
  ...
  // HandoverListRow — GET /handover/pending + /handover/patient reduced
  // projection (history adds acknowledged_at). jsonb arrays preserved.
  HandoverListRow: {
    type: 'object',
    additionalProperties: true,
    required: ['id', 'patient_uid', 'shift'],
    properties: {
      id: { type: 'integer' },
      patient_uid: { type: 'string', format: 'uuid' },
      ward: { type: 'string', nullable: true },
  ...
  // HandoverDraft — POST /handover/generate (AI). LLM-merged keys → LOOSE; minimal
  // required core. ai_metadata reuses the shared aiMetadata shape.
  HandoverDraft: {
    type: 'object',
    additionalProperties: true,
    required: ['patient_uid', 'patient_summary', 'ai_metadata'],
    properties: {
      patient_uid: { type: 'string', format: 'uuid' },
  ...
  // =========================================================================
  // mar sub-domain — Voice-to-SOAP
  // =========================================================================
  // VoiceNoteConfig — GET /voice-note/config deterministic STT config + voice_note
  // sub-object. STRICT-ish (loose to tolerate config additions).
  VoiceNoteConfig: {
    type: 'object',
    additionalProperties: true,
    required: ['provider', 'configured'],
    properties: {
      provider: { type: 'string', enum: STT_PROVIDER },
      model: { type: 'string', nullable: true },
      configured: { type: 'boolean' },
      reason: { type: 'string', nullable: true },
      external_call: { type: 'boolean' },
      endpoint_configured: { type: 'boolean' },
      api_key_configured: { type: 'boolean', nullable: true },
      tenant_region: { type: 'string', nullable: true },
      ...
      voice_note: {
        ...
        properties: {
          ...
          audio_capture_allowed: { type: 'boolean' },
          blocking_reason: { type: 'string', nullable: true },
          decision_support_only: { type: 'boolean' },
          human_review_required: { type: 'boolean' },
          patient_dispatch_allowed: { type: 'boolean' },
          consent_policy_required: { type: 'boolean' },
          retention_days: { type: 'number' },
          approval_policy: { type: 'string', nullable: true },
  ...
  // VoiceNote — voice-note/transcribe row + voice-note/my items. metadata jsonb
  // → object. LOOSE-ish.
  VoiceNote: {
    type: 'object',
    additionalProperties: true,
    required: ['id', 'patient_uid', 'transcript_status'],
    properties: {
      id: { type: 'integer' },
      tenant_id: { type: 'string', format: 'uuid' },
      recorded_by: { type: 'string', format: 'uuid', nullable: true },
      ...
      audio_storage_key: { type: 'string', nullable: true },
      audio_mime: { type: 'string', nullable: true },
      audio_duration_seconds: { type: 'number', nullable: true },
      stt_provider: { type: 'string', nullable: true },
      stt_model: { type: 'string', nullable: true },
      stt_language: { type: 'string', nullable: true },
      transcript: { type: 'string', nullable: true },
      transcript_status: { type: 'string', enum: TRANSCRIPT_STATUS },
      transcript_failure_reason: { type: 'string', nullable: true },
      metadata: { type: 'object', additionalProperties: true },
      generation_id: { type: 'integer', nullable: true },
      created_at: { type: 'string', format: 'date-time' },
      updated_at: { type: 'string', format: 'date-time' },
  ...
  // VoiceNoteList — GET /voice-note/my {voice_notes, count}.
  VoiceNoteList: {
    type: 'object',
    additionalProperties: true,
    required: ['voice_notes', 'count'],
    properties: {
      voice_notes: { type: 'array', items: { $ref: '#/components/schemas/VoiceNote' } },
      count: { type: 'integer' },
    },
  },

  // VoiceSoapDraft — POST /voice-note/{id}/generate-soap (AI). draft OPAQUE.
  VoiceSoapDraft: {
    type: 'object',
    additionalProperties: true,
    required: ['draft', 'review_status'],
    properties: {
      draft: aiDraft,
      module_key: { type: 'string' },
      prompt_version: { type: 'string' },
      ...
      safety_flags: { type: 'array', items: aiSafetyFlag },
      ai_metadata: aiMetadata,
      review_status: { type: 'string', enum: AI_DRAFT_REVIEW_STATUS },
      generation_id: { type: 'integer', nullable: true },
      voice_note_id: { type: 'integer', nullable: true },
      requires_signoff: { type: 'boolean' },
    },
  },

  // =========================================================================
  // mar sub-domain — Clinical safety AI
  // =========================================================================
  // DeteriorationScore — POST /safety/deterioration/{patientUid} (heuristic).
  // contributors has TWO branches (normal vs no_vitals) → LOOSE; latest_vital
  // opaque. Required core {patient_uid, score, band}.
  DeteriorationScore: {
    type: 'object',
    additionalProperties: true,
    required: ['patient_uid', 'score', 'band'],
    properties: {
      snapshot_id: { type: 'integer', nullable: true },
      patient_uid: { type: 'string', format: 'uuid' },
      admission_id: { type: 'integer', nullable: true },
      score: { type: 'number' },
      band: { type: 'string', enum: DETERIORATION_BAND },
      contributors: { type: 'object', additionalProperties: true },
      recommendations: {
        type: 'array',
        items: {
          type: 'object',
      ...
      vitals_sample_count: { type: 'number' },
      module_key: { type: 'string' },
      decision_support_only: { type: 'boolean' },
    },
  },

  // PolypharmacyReview — POST /safety/polypharmacy (rules + AI). findings +
  // echoed `medications` LOOSE. combined_severity reuses AI_SAFETY_SEVERITY.
  PolypharmacyReview: {
    type: 'object',
    additionalProperties: true,
    required: ['medications', 'rule_findings', 'ai_findings', 'combined_severity'],
    properties: {
      review_id: { type: 'integer', nullable: true },
      tenant_id: { type: 'string', format: 'uuid' },
      patient_uid: { type: 'string', format: 'uuid', nullable: true },
      ...
      rule_findings: {
        ...
        items: {
          ...
          properties: {
            severity: { type: 'string', enum: AI_SAFETY_SEVERITY },
            source: { type: 'string', enum: POLYPHARMACY_FINDING_SOURCE },
            code: { type: 'string' },
            message: { type: 'string' },
          },
        },
      ...
      combined_severity: { type: 'string', enum: AI_SAFETY_SEVERITY },
      provider: { type: 'string' },
      used_ai: { type: 'boolean' },
  ...
  // =========================================================================
  // mar sub-domain — Ambient documentation
  // =========================================================================
  // AmbientEncounter — POST /ambient/encounters (AI). draft + talk_time OPAQUE.
  AmbientEncounter: {
    type: 'object',
    additionalProperties: true,
    required: ['module_key', 'draft', 'review_status'],
    properties: {
      encounter_id: { type: 'integer', nullable: true },
      generation_id: { type: 'integer', nullable: true },
      draft: aiDraft,
      ...
      talk_time: { type: 'object', additionalProperties: true },
      diarization_status: { type: 'string', nullable: true },
      diarization_provider: { type: 'string', nullable: true },
      diarization_reason: { type: 'string', nullable: true },
      used_ai: { type: 'boolean' },
      provider: { type: 'string' },
      module_key: { type: 'string' },
      review_status: { type: 'string', enum: AI_DRAFT_REVIEW_STATUS },
  ...
  // AmbientEncounterListRow — GET /ambient/encounters items.
  AmbientEncounterListRow: {
    type: 'object',
    additionalProperties: true,
    required: ['id', 'patient_uid'],
    properties: {
      ...
      clinician_uid: { type: 'string', format: 'uuid', nullable: true },
      recorded_by: { type: 'string', format: 'uuid', nullable: true },
      stt_provider: { type: 'string', nullable: true },
      stt_language: { type: 'string', nullable: true },
      diarization_provider: { type: 'string', nullable: true },
      speaker_count: { type: 'number' },
      transcript_status: { type: 'string', nullable: true },
      generation_id: { type: 'integer', nullable: true },
      created_at: { type: 'string', format: 'date-time' },
    },
  },

  // AmbientEncounterList — GET /ambient/encounters {encounters, count}.
  AmbientEncounterList: {
    type: 'object',
    additionalProperties: true,
    required: ['encounters', 'count'],
    properties: {
      encounters: { type: 'array', items: { $ref: '#/components/schemas/AmbientEncounterListRow' } },
      count: { type: 'integer' },
    },
  },

  // =========================================================================
  // mar sub-domain — response envelopes
  // =========================================================================
  // MAR core. schedule → bare MarRow[] (empty meds → []). Mutations → single rows.
  EmrMarScheduleResponse: listEnvelope('MarRow'),
  EmrMarRowResponse: envelope('MarRow'),
  EmrMarAdministeredResponse: envelope('MarAdministeredRow'),
  EmrMarScannedResponse: envelope('MarScannedRow'),
  EmrMarFiveRightsResponse: envelope('MarFiveRightsResult'),
  EmrMarPatientListResponse: listEnvelope('MarListRow'),
  EmrMarOverdueListResponse: listEnvelope('MarOverdueRow'),
  EmrMarDueListResponse: listEnvelope('MarDueRow'),
  // Drug chart.
  EmrDrugChartResponse: envelope('DrugChart'),
  // NEWS2. record → data can be NULL (no usable vital) — nullable data envelope.
  EmrNews2RecordResponse: {
    type: 'object',
    ...
    properties: {
      ...
      data: {
        nullable: true,
        allOf: [{ $ref: '#/components/schemas/News2Record' }],
      },
    },
  },
  EmrNews2HistoryResponse: envelope('News2History'),
  // Handover.
  EmrHandoverDraftResponse: envelope('HandoverDraft'),
  EmrHandoverResponse: envelope('Handover'),
  EmrHandoverListResponse: listEnvelope('HandoverListRow'),
  // Voice-to-SOAP.
  EmrVoiceNoteConfigResponse: envelope('VoiceNoteConfig'),
  EmrVoiceNoteResponse: envelope('VoiceNote'),
  EmrVoiceNoteListResponse: envelope('VoiceNoteList'),
  EmrVoiceSoapDraftResponse: envelope('VoiceSoapDraft'),
  // Clinical safety AI.
  EmrDeteriorationResponse: envelope('DeteriorationScore'),
  EmrPolypharmacyResponse: envelope('PolypharmacyReview'),
  // Ambient documentation.
  EmrAmbientEncounterResponse: envelope('AmbientEncounter'),
  EmrAmbientEncounterListResponse: envelope('AmbientEncounterList'),

  // =========================================================================
  // mar sub-domain — request bodies (all LOOSE; raw intake convention)
  // =========================================================================
  // POST /mar/schedule — medications array (may be empty → []).
  EmrMarScheduleRequest: {
    type: 'object', additionalProperties: true,
  ...
  // POST /mar/{id}/administer — loose intake (witness/override/notes optional).
  EmrMarAdministerRequest: { type: 'object', additionalProperties: true },
  // POST /mar/{id}/administer-with-scan — barcodes required, override optional.
  EmrMarScanRequest: {
    type: 'object', additionalProperties: true,
    required: ['patient_barcode', 'medication_barcode'],
    properties: {
      patient_barcode: { type: 'string' },
      medication_barcode: { type: 'string' },
      override_reason: { type: 'string' },
    },
  },
  // POST /mar/verify — loose dry-run intake.
  EmrMarVerifyRequest: { type: 'object', additionalProperties: true },
  // POST /mar/{id}/hold — hold_reason required.
  EmrMarHoldRequest: {
    type: 'object', additionalProperties: true,
    required: ['hold_reason'],
    properties: { hold_reason: { type: 'string' } },
  },
  // POST /mar/{id}/miss — notes optional.
  EmrMarMissRequest: {
    type: 'object', additionalProperties: true,
    properties: { notes: { type: 'string' } },
  },
  // POST /news2/record — loose vitals intake.
  EmrNews2RecordRequest: { type: 'object', additionalProperties: true },
  // POST /handover/generate — patient_uid required (AI).
  EmrHandoverGenerateRequest: {
    type: 'object', additionalProperties: true,
    required: ['patient_uid'],
    properties: { patient_uid: { type: 'string', format: 'uuid' } },
  },
  // POST /handover — loose; shift enum-bound when present.
  EmrCreateHandoverRequest: {
    type: 'object', additionalProperties: true,
    properties: {
      patient_uid: { type: 'string', format: 'uuid' },
      shift: { type: 'string', enum: HANDOVER_SHIFT },
  ...
  // POST /safety/deterioration/{patientUid} — loose intake.
  EmrDeteriorationRequest: { type: 'object', additionalProperties: true },
  // POST /safety/polypharmacy — medications array required.
  EmrPolypharmacyRequest: {
    type: 'object', additionalProperties: true,
    required: ['medications'],
    properties: {
      medications: { type: 'array', items: { type: 'object', additionalProperties: true } },
  ...
  // POST /ambient/encounters — transcript_segments array (AI).
  EmrAmbientEncounterRequest: {
    type: 'object', additionalProperties: true,
  ...
  // POST /progress-notes — loose; note/content/body/text aliases (clinicalNotesService).
  EmrProgressNoteRequest: { type: 'object', additionalProperties: true },
};
...
const EMR_ONLY_OPS = [
  ...
  // -------------------------------------------------------------------------
  // mar sub-domain (clinicalRoutes.js reached via the /api/v1/emr/mar rewrite —
  // EMR-only mount, NOT aliased to /api/v1/admissions). 26 ops / 25 paths
  // (ambient/encounters has GET+POST). The mount rewrite PREPENDS `/mar`, so the
  // doubled `/mar/mar/...` segment is real (verified against the live spec).
  // Every response wraps via success(res,data,…). jsonb columns → loose objects;
  // AI ops (handover/generate, voice-note generate-soap, ambient encounters)
  // type the envelope normally + leave the inner draft/talk_time loose.
  // progress-notes REUSES the notes-diagnosis EmrClinicalNoteResponse.
  // -------------------------------------------------------------------------
  // MAR core — schedule (bare MarRow[], empty meds → []) + mutations (single rows).
  ['POST /mar/mar/schedule', { request: 'EmrMarScheduleRequest', response: 'EmrMarScheduleResponse' }],
  ['POST /mar/mar/{id}/administer', { request: 'EmrMarAdministerRequest', response: 'EmrMarAdministeredResponse' }],
  ['POST /mar/mar/{id}/administer-with-scan', { request: 'EmrMarScanRequest', response: 'EmrMarScannedResponse' }],
  ['POST /mar/mar/verify', { request: 'EmrMarVerifyRequest', response: 'EmrMarFiveRightsResponse' }],
  ['POST /mar/mar/{id}/hold', { request: 'EmrMarHoldRequest', response: 'EmrMarRowResponse' }],
  ['POST /mar/mar/{id}/miss', { request: 'EmrMarMissRequest', response: 'EmrMarRowResponse' }],
  // MAR LIST reads (bare arrays; pagination/scope in meta where present).
  ['GET /mar/mar/patient/{patientUid}', { response: 'EmrMarPatientListResponse' }],
  ['GET /mar/mar/overdue', { response: 'EmrMarOverdueListResponse' }],
  ['GET /mar/mar/due', { response: 'EmrMarDueListResponse' }],
  // Drug chart — composite detail.
  ['GET /mar/drug-chart/admission/{id}', { response: 'EmrDrugChartResponse' }],
  // NEWS2 — record (nullable data) + patient history {scores,trend}.
  ['POST /mar/news2/record', { request: 'EmrNews2RecordRequest', response: 'EmrNews2RecordResponse' }],
  ['GET /mar/news2/patient/{patientUid}', { response: 'EmrNews2HistoryResponse' }],
  // Handover — generate (AI; draft loose) + create + acknowledge + lists.
  ['POST /mar/handover/generate', { request: 'EmrHandoverGenerateRequest', response: 'EmrHandoverDraftResponse' }],
  ['POST /mar/handover', { request: 'EmrCreateHandoverRequest', response: 'EmrHandoverResponse' }],
  ['POST /mar/handover/{id}/acknowledge', { response: 'EmrHandoverResponse' }],
  ['GET /mar/handover/pending', { response: 'EmrHandoverListResponse' }],
  ['GET /mar/handover/patient/{patientUid}', { response: 'EmrHandoverListResponse' }],
  // Voice-to-SOAP — config + transcribe (multipart) + generate-soap (AI) + my list.
  ['GET /mar/voice-note/config', { response: 'EmrVoiceNoteConfigResponse' }],
  ['POST /mar/voice-note/transcribe', { response: 'EmrVoiceNoteResponse' }],
  ['POST /mar/voice-note/{id}/generate-soap', { response: 'EmrVoiceSoapDraftResponse' }],
  ['GET /mar/voice-note/my', { response: 'EmrVoiceNoteListResponse' }],
  // Clinical safety AI — deterioration + polypharmacy.
  ['POST /mar/safety/deterioration/{patientUid}', { request: 'EmrDeteriorationRequest', response: 'EmrDeteriorationResponse' }],
  ['POST /mar/safety/polypharmacy', { request: 'EmrPolypharmacyRequest', response: 'EmrPolypharmacyResponse' }],
  // Ambient documentation — create (AI) + list.
  ['POST /mar/ambient/encounters', { request: 'EmrAmbientEncounterRequest', response: 'EmrAmbientEncounterResponse' }],
  ['GET /mar/ambient/encounters', { response: 'EmrAmbientEncounterListResponse' }],
  // Progress notes alias — delegates to clinicalNotesService.createNote; REUSE
  // the notes-diagnosis single-note envelope (ClinicalNote shape).
  ['POST /mar/progress-notes', { request: 'EmrProgressNoteRequest', response: 'EmrClinicalNoteResponse' }],
];

...
async function main() {
  ...
  try {
    ...
    const existing = await client.query(
      `SELECT definition.id AS workflow_definition_id, governance.id AS governance_id
    ...
    if (existing.rows.length > 1) throw new Error('More than one approved Referral definition is active');
    if (existing.rows[0]) {
    ...
    const conflict = await client.query(
      `SELECT id FROM workflow_definitions
        WHERE tenant_id = $1::uuid AND workflow_key = $2::text
          AND version = $3::integer AND is_active = TRUE`,
      [tenantId, compiled.workflow_key, compiled.version],
    );
    if (conflict.rows.length > 0) throw new Error('An unmatched active Referral definition already exists');
    if (!apply) {
      await client.query('ROLLBACK');
      process.stdout.write(`${JSON.stringify({
        mode: 'dry_run',
    ...
    const definition = await client.query(
      `INSERT INTO workflow_definitions
       ...
       VALUES ($1::uuid, $2::text, $3::integer, 'Referral request to closure',
      ...
      [
        ...
        compiled.version,
        JSON.stringify(REFERRAL_REQUEST_TO_CLOSURE_DEFINITION.steps),
        JSON.stringify(REFERRAL_REQUEST_TO_CLOSURE_DEFINITION.triggers),
        JSON.stringify(REFERRAL_REQUEST_TO_CLOSURE_DEFINITION.defaults),
        operationalOwnerUid,
      ],
    );
    ...
    const approval = await client.query(
      `INSERT INTO approvals
       ...
       VALUES ($1::uuid, 'care_pathway_definition_governance',
          ...
          $3::uuid, NOW(), $3::uuid,
          jsonb_build_object('care_pathway_definition_governance',
    ...
    const governance = await client.query(
      `INSERT INTO care_pathway_definition_governance
       ...
       VALUES ($1::uuid, $2::integer, $3::uuid, $4::uuid, 'approved', $5::integer,

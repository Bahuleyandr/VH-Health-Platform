...
async function post(baseUrl, endpoint, body = {}, { outcome = null, workflowId = '77' } = {}) {
...
async function main() {
  ...
  try {
    ...
    await post(info.baseUrl, 'claim/submit', { payload: 'k.l.m.n.o' }, { outcome: 'approve', workflowId: '101' });
    await post(info.baseUrl, 'claim/submit', { payload: 'p.q.r.s.t' }, { outcome: 'query', workflowId: '102' });
    ...
    assert(log.requests.length === 5, `expected 5 recorded requests, got ${log.requests.length}`);

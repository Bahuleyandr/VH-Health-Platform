...
function sendOperationalError(res, err, fallback = 'Request failed') {
  const statusCode = err?.statusCode || HTTP_STATUS.INTERNAL_SERVER_ERROR;
  const details = err?.code
    ? {
        code: err.code,
    ...
    : null;
  return error(res, err?.message || fallback, statusCode, details);
}
...
export class UserController {
    ...
    try {
      ...
      if (!isPrivilegedActor) {
        ...
        if (!tokenPhone && !tokenUid) {
          return error(
            res,
            'Authenticated identity required for profile self-service',
            HTTP_STATUS.FORBIDDEN
          );
        }
        const bodyPhone =
          body.phone || body.phoneNumber ? normalizePhone(body.phone || body.phoneNumber) : null;
      ...
      const result = await UserService.createOrUpdateProfile(body, req.user?.uid || 'system', {
        isPrivilegedActor,
  ...
  static async deleteOwnAccount(req, res) {
    try {
      ...
      const result = await UserService.deleteOwnAccount({
        ...
        reason: req.body.reason || 'patient_self_service',
        requestId: req.id,
        ipAddress: req.ip,
      ...
      logger.error('Delete Own Account Controller Error:', err);
      if (err?.isOperational || err?.statusCode) {
        return sendOperationalError(res, err, 'Account deletion failed');
      }
      error(res, 'Failed to delete account', HTTP_STATUS.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  static async updateUser(req, res) {
    try {
      ...
      const user = await UserService.updateUser(req.params.identifier, req.body, req.user?.uid);
  ...
  static async bulkImportUsers(req, res) {
    try {
      ...
      const result = await UserService.bulkImportUsers(req.body.users, req.user?.uid);

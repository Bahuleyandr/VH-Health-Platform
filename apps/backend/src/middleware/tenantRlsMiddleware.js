...
//
// Every request scopes to req.tenantId. SUPER_ADMIN `x-tenant-id`
// overrides are resolved by tenantContextMiddleware before this point and
// still get a tenant-scoped context; they are not an RLS bypass. Explicit
// cross-tenant aggregators use runWithSuperAdmin() outside the request path.
// If req.tenantId is unset (cron, health probes, public endpoints), we run
// the chain in the empty context — the prisma auto-wrapper short-circuits
// and passes through.
//
...
export default function tenantRlsMiddleware(req, _res, next) {
  ...
  runInTenantContext(tenantId, () => next());
}

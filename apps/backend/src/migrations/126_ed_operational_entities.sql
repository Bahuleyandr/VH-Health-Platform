-- Migration 126: Phase D4 — ED operational entities.
--
-- HEALTHCARE_AI_SPEC_AUDIT.md §13: AI triage exists
-- (clinical_ai_ed_triage_predictions, edTriageBoardingService) but the
-- operational ED entities (visit, ambulance, MLC) are missing.
--
-- Tables:
--   1. emergency_visits         — top-level ED visit record. Distinct
--                                   from generic appointments because
--                                   walk-in / stretcher arrivals don't
--                                   start at the appointments table.
--   2. triage_assessments       — ESI / Manchester / CTAS triage
--                                   results per visit; multiple
--                                   re-assessments allowed.
--   3. ambulance_requests       — ambulance dispatch request +
--                                   en-route status; can be linked to
--                                   an emergency_visit on arrival.
--   4. mlc_records              — medico-legal case records (RTA,
--                                   assault, poisoning, attempt to
--                                   suicide, dog bite). Indian regs
--                                   require these to flag the police-
--                                   report obligation explicitly.
...
CREATE INDEX IF NOT EXISTS idx_ed_visits_open
  ...
  WHERE status NOT IN ('discharged', 'transferred', 'left_against_advice', 'lwbs', 'expired', 'archived');
...
CREATE TABLE IF NOT EXISTS ambulance_requests (
  ...
  request_kind                VARCHAR(40) NOT NULL DEFAULT 'pickup'
    CHECK (request_kind IN ('pickup', 'transfer_out', 'inter_facility', 'home_to_hospital', 'air_evac', 'other')),

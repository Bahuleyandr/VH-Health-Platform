-- 743_care_pathway_actionable_roles_pharmacist_sync.sql
--
-- Adding PHARMACIST to the canonical legacy role registry moved that role
-- earlier in the derived CLINICAL_STAFF_ROUTE_ROLES policy order. Keep the
-- database-side task-owner policy order-exact with routeRolePolicy.js; the
-- care-pathway schema conformance suite compares these arrays directly.
...
AS $$
  SELECT CASE
    ...
    ELSE ARRAY[
      ...
      'CMO', 'CNO', 'CONSULTANT', 'JUNIOR_DOCTOR', 'RESIDENT', 'PHARMACIST',

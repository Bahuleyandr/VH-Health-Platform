-- Migration 759: parenthesise the bare CASE in two escalation snapshot guards.
--
-- 744 and 745 each ship a trigger function whose IF condition contains a BARE
-- CASE at parenthesis depth 0:
--
--     IF ... OR CASE WHEN pg_input_is_valid(...) THEN ... ELSE TRUE END
--     THEN ...
--
-- plpgsql reads an IF condition by scanning for the terminator keyword THEN at
-- depth 0, so it stops at the CASE's own THEN. The condition is left as
-- "... OR CASE WHEN pg_input_is_valid(...)", which is incomplete, and the server
-- raises 42601 "syntax error at end of input". Wrapping the CASE in parentheses
-- puts its THEN below depth 0 and the scanner continues to the real terminator.
--
-- Why this was never caught: 000_baseline.sql line 45 issues a SESSION-level
-- `SET check_function_bodies = false`, and ci-setup-db.mjs applies every
-- migration through ONE long-lived connection, so body validation stayed off for
-- every migration after the baseline. The defective CREATE FUNCTION therefore
-- succeeded everywhere while being uncompilable. plpgsql compiles a body lazily
-- on first call, so both triggers would RAISE the first time they fired. This
-- migration is the runtime fix; the leak itself is closed separately in
-- ci-setup-db.mjs so future bodies are actually validated.
--
-- The bodies below are the shipped 744/745 definitions with the parenthesis pair
-- added and NOTHING else changed -- these are clinical guard predicates and must
-- not drift. Verified by asserting the rewrite differs from the original by
-- exactly the two added characters.
--
-- Body checking is deliberately NOT relaxed here: these functions must compile.

BEGIN;

-- ---- mar_medication_exception_escalation_snapshot_guard (originally shipped in 744_medication_inventory_billing_mar_closure.sql) ----
...
-- ---- clinical_alert_delivery_recovery_escalation_snapshot_guard (originally shipped in 745_clinical_alert_delivery_obligations.sql) ----

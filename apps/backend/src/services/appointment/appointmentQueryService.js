...
async function resolveDoctorDepartmentForUser(userId) {
  const doctorId = Number.parseInt(userId, 10);
  ...
  try {
    ...
    logger.warn('Doctor department lookup failed:', e?.message);
    return null;
  }
}
    ...
    try {
      ...
      if (userRole === 'DOCTOR') {
        ...
        if (doctorDepartment) {
          doctorScope.push({
            doctor_id: null,
            department: { equals: doctorDepartment, mode: 'insensitive' },
          });
        }
      ...
      if (filters.department) {
        where.department = {
          equals: String(filters.department).trim(),
          mode: 'insensitive',
        };
      }

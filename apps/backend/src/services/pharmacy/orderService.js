...
export const createOrder = async (orderData) => {
  ...
  await emitPharmacyOrderEvent({
    ...
    eventStatus: ORDER_STATUS.PENDING,
    payload: { delivery_type: resolvedDeliveryType, source: 'legacy_order_service' },
  });
...
export const updateOrderStatus = async (orderId, status, notes, updatedBy, updatedByRole, tenantId = null) => {
  const validStatuses = Object.values(ORDER_STATUS);
  if (!validStatuses.includes(status)) throw new Error('INVALID_STATUS');

  // pharmacy_orders is RLS-protected (tenant_id, tenant_isolation policy). A bare
  // prisma.$transaction leaves app.current_tenant_id unset → permissive branch →
  // cross-tenant rows reachable under FOR UPDATE / UPDATE. Scope the whole tx to
  // the caller's tenant (threaded from req.tenantId; DEFAULT_TENANT_ID is the
  // single-tenant production floor when unset).
  return setTenantTx(requireTenantId(tenantId), async (tx) => {
    ...
    await emitPharmacyOrderEvent({
      ...
      payload: { notes: notes || null, source: 'legacy_order_service' },
    });

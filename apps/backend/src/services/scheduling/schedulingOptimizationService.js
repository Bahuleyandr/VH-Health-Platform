...
export async function upsertTemplate({
  ...
  const rows = await prisma.$queryRawUnsafe(
    `INSERT INTO provider_availability_templates
       (doctor_id, weekday, start_time, end_time, slot_minutes, location, effective_from, effective_to, created_by)
     VALUES ($1, $2, $3::time, $4::time, $5, $6, COALESCE($7::date, CURRENT_DATE), $8::date, $9::uuid)
     RETURNING *`,
    doctor, day, startTime, endTime, Number.parseInt(slotMinutes, 10) || 15,
    location, effectiveFrom, effectiveTo, context.actorUid || null,
  );
  return rows[0];
}

export async function listTemplates(doctorId) {
  return prisma.$queryRawUnsafe(
    `SELECT * FROM provider_availability_templates
      WHERE doctor_id = $1 AND is_active ORDER BY weekday, start_time`,
    doctorId,
  );
}

export async function recordLeave({ doctorId, startsOn, endsOn, reason = null } = {}, context = {}) {
  const rows = await prisma.$queryRawUnsafe(
    `INSERT INTO provider_leaves (doctor_id, starts_on, ends_on, reason, approved_by)
     VALUES ($1, $2::date, $3::date, $4, $5::uuid) RETURNING *`,
    Number.parseInt(doctorId, 10), startsOn, endsOn, reason, context.actorUid || null,
  );
  return rows[0];
}
 ...
 */
export async function getSlotGrid({ doctorId, date } = {}) {
  const doctor = Number.parseInt(doctorId, 10);
  ...
  const [templates, leaves, booked] = await Promise.all([
    prisma.$queryRawUnsafe(
      `SELECT start_time::text, end_time::text, slot_minutes, location
        ...
        WHERE doctor_id = $1 AND is_active
          AND weekday = EXTRACT(DOW FROM $2::date)::int
    ...
    prisma.$queryRawUnsafe(
      `SELECT id, reason FROM provider_leaves
        WHERE doctor_id = $1 AND $2::date BETWEEN starts_on AND ends_on LIMIT 1`,
      doctor, date,
    ),
    prisma.$queryRawUnsafe(
...
export async function addToWaitlist({
  ...
  const rows = await prisma.$queryRawUnsafe(
    `INSERT INTO appointment_waitlist
       (patient_uid, doctor_id, preferred_date, preferred_window, priority, notes, created_by)
     VALUES ($1::uuid, $2, $3::date, $4, $5, $6, $7::uuid) RETURNING *`,
    patientUid, Number.parseInt(doctorId, 10), preferredDate, preferredWindow,
 ...
 */
export async function fillWaitlist({ doctorId, date } = {}, context = {}) {
  const grid = await getSlotGrid({ doctorId, date });
  ...
  const waiting = await prisma.$queryRawUnsafe(
    `SELECT * FROM appointment_waitlist
      WHERE doctor_id = $1 AND status = 'waiting'
        AND (preferred_date IS NULL OR preferred_date = $2::date)
      ORDER BY priority ASC, created_at ASC
      LIMIT $3::int`,
    Number.parseInt(doctorId, 10), date, freeSlots.length,
  );
  ...
  for (const [index, entry] of waiting.entries()) {
    ...
    const updated = await prisma.$queryRawUnsafe(
      `UPDATE appointment_waitlist SET
       ...
       WHERE id = $1 AND status = 'waiting' RETURNING id, patient_uid`,
      entry.id, JSON.stringify({ date, start: slot.start, end: slot.end, location: slot.location }),
    );
    if (updated.length) {
...
export async function resolveWaitlistEntry(id, { status } = {}) {
  ...
  const rows = await prisma.$queryRawUnsafe(
    `UPDATE appointment_waitlist SET status = $2, resolved_at = NOW(), updated_at = NOW()
      WHERE id = $1 AND status IN ('waiting', 'offered') RETURNING *`,
    Number.parseInt(id, 10), status,
  );
...
export async function sweepWaitlists() {
  const targets = await prisma.$queryRawUnsafe(
    `SELECT DISTINCT doctor_id FROM appointment_waitlist WHERE status = 'waiting' LIMIT 50`,
  );
  ...
  for (const target of targets) {
    for (const date of dates) {
      try {
        const result = await fillWaitlist({ doctorId: target.doctor_id, date });
...
export async function createResource({ kind, name, location = null } = {}) {
...
export async function bookResource({
  ...
  return prisma.$transaction(async (tx) => {
    ...
    await tx.$queryRawUnsafe(`SELECT id FROM bookable_resources WHERE id = $1 AND is_active FOR UPDATE`, id)
      .then((rows) => {
    ...
    const rows = await tx.$queryRawUnsafe(
      `INSERT INTO resource_bookings
         (resource_id, starts_at, ends_at, booked_for_type, booked_for_id, patient_uid, booked_by, notes)
       VALUES ($1, $2::timestamptz, $3::timestamptz, $4, $5, $6::uuid, $7::uuid, $8)
       RETURNING *`,
      id, startsAt, endsAt, bookedForType, bookedForId, patientUid, context.actorUid || null, notes,
    );
    return rows[0];
  });
}

export async function listResourceSchedule({ resourceId, date } = {}) {
  return prisma.$queryRawUnsafe(
    `SELECT b.*, r.name AS resource_name, r.kind
       FROM resource_bookings b JOIN bookable_resources r ON r.id = b.resource_id
      WHERE b.resource_id = $1 AND b.status = 'booked'
        AND b.starts_at::date = $2::date
      ORDER BY b.starts_at`,
    Number.parseInt(resourceId, 10), date,
  );
}

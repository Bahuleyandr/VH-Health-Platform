  ...
  return setTenantTx(TENANT_ID, async (tx) => {
    const rows = await tx.$queryRawUnsafe(
      `INSERT INTO interop_messages
       ...
       VALUES ($1::uuid, $2::integer, $3::integer, $4::text, 'csv',
               $5::text, $6::text, $7::text, $8::text, 'live', 'live',
...
async function recover(message) {
  ...
  const command = {
    ...
    actor_uid: ACTOR_UID,
    owner_reason: `The accountable owner classified the late ${direction} CSV message for review.`,
    evidence: { outage_reconciliation_id: `csv-recon-${direction}-${SUFFIX}` },
  };
  const envelope = {
    tenantId: TENANT_ID,
    offsetId: offset.offset_id,
    ...
    duplicateKey: `csv:${channelId}:${direction}:${systemId}:${PAYLOAD_HASH}`,
    command,
  };
...
describeIfDb('C6.1-E I05 CSV recovery adapter', () => {
  beforeAll(async () => {
    await prisma.$executeRawUnsafe(
      `INSERT INTO tenants (id, slug, name) VALUES ($1::uuid, $2::text, 'C6.1-E I05 CSV tenant')`,
      TENANT_ID,
      `c61e-i05-csv-${SUFFIX}`,
    );
    await prisma.$executeRawUnsafe(
    ...
    await setTenantTx(TENANT_ID, async (tx) => {
      const systems = await tx.$queryRawUnsafe(
        `INSERT INTO interop_systems (tenant_id, system_key, display_name, kind, direction, status)
         VALUES ($1::uuid, $2::text, 'CSV recovery target', 'vh_backend', 'bidirectional', 'active') RETURNING id`,
        TENANT_ID,
  ...
  test.each([
    ['inbound', 'pending_review', 'i05_csv_inbound_pending_review', 'backend.interop.csv'],
    ['outbound', 'send_held', 'i05_csv_outbound_send_held', 'external.csv.http'],

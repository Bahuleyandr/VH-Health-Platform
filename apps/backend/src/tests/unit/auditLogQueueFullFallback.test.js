 ...
 * the entry was DROPPED with only a `logger.warn('Audit log queue full, ...')`
...
describe('auditLogMiddleware queue-full durability', () => {
  it('writes the dropped entry to the Winston file fallback when the queue is full (never silently lost)', async () => {
    ...
    // No queue-full warning yet — we are exactly AT capacity, not over.
    const dropWarnsBefore = warnMock.mock.calls.filter(
      (c) => typeof c[0] === 'string' && c[0].includes('queue full'),
    );
    expect(dropWarnsBefore).toHaveLength(0);
    ...
    const dropCalls = warnMock.mock.calls.filter(

...
const {
  ...
  scoreDeterioration,
} = await import('../../services/ai/deteriorationEarlyWarningService.js');
...
test('a clean empty vitals result is explicit insufficient data with no risk score or band', async () => {
...
test('snapshot persistence failure rejects instead of returning an unaudited clinical score', async () => {
  queryRawUnsafe
    .mockResolvedValueOnce([VITAL])
...
test('snapshot list faults reject instead of looking like an empty dashboard', async () => {
  queryRawUnsafe.mockRejectedValueOnce(new Error('relation "clinical_ai_deterioration_snapshots" does not exist'));

  await expect(listDeteriorationSnapshots({ tenantId: TENANT_ID }))
    .rejects.toThrow('clinical_ai_deterioration_snapshots');
});

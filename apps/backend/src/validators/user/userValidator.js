...
export const userValidation = [phoneValidation, ...profileFieldValidation];
...
export const searchValidation = [
  ...
  query('search')
    .optional()
    .isLength({
      min: USER_CONFIG.SEARCH.MIN_QUERY_LENGTH,
      max: USER_CONFIG.SEARCH.MAX_QUERY_LENGTH
    })
    .withMessage(
      `Search query must be ${USER_CONFIG.SEARCH.MIN_QUERY_LENGTH}-${USER_CONFIG.SEARCH.MAX_QUERY_LENGTH} characters`
    ),
  query('status')
    .optional()
...
export const roleValidation = [
  param('role').isIn(Object.values(USER_CONFIG.ROLES)).withMessage('Invalid role specified')
];
...
export const userSearchValidation = [
  query('query')
    ...
    .withMessage(
      `Query must be ${USER_CONFIG.SEARCH.MIN_QUERY_LENGTH}-${USER_CONFIG.SEARCH.MAX_QUERY_LENGTH} characters`
    ),
  query('role').optional().isIn(Object.values(USER_CONFIG.ROLES)).withMessage('Invalid role'),
  query('department').optional().isString().withMessage('Department must be a string'),
  query('registeredAfter').optional().isISO8601().withMessage('Invalid date format'),
  query('registeredBefore').optional().isISO8601().withMessage('Invalid date format'),
  query('lastLoginAfter').optional().isISO8601().withMessage('Invalid date format'),
  query('ageMin')
    .optional()
    .isInt({ min: 0, max: 150 })
...
export const statusChangeValidation = [
  param('identifier').notEmpty().withMessage('User identifier is required'),
  body('status').isIn(Object.values(USER_CONFIG.USER_STATUS)).withMessage('Invalid status'),
  body('reason')
    .optional()
    .isLength({ max: 500 })

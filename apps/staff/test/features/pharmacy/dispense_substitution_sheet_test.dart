...
CompositionAlternativesResult _alts() => CompositionAlternativesResult.fromJson(const {
      'selected': {
        'catalog_id': 101, 'composition_id': 3, 'strength': '625 mg',
        'strength_key': '625mg', 'form': 'tablet', 'form_key': 'tablet', 'release_key': 'ir',
      },
      'groups': [
        {
          'strength': '625 mg', 'strength_key': '625mg', 'form': 'tablet', 'form_key': 'tablet',
          'matched': true,
          'items': [
            {
              'catalog_id': 202, 'name': 'Clavam 625', 'manufacturer': 'Alkem',
              'strength': '625 mg', 'strength_key': '625mg', 'form': 'tablet', 'form_key': 'tablet',
              'stock_quantity': 14, 'availability_status': 'in_stock', 'substitutable': true,
            },
          ],
        },
      ],
      'alternatives': [
        {'catalog_id': 202, 'name': 'Clavam 625', 'availability_status': 'in_stock', 'substitutable': true},
      ],
    });

void main() {
  testWidgets('full flow: load context → swap → batch → dispense with correct args', (tester) async {
    Map<String, dynamic>? captured;
    await tester.pumpWidget(_host(DispenseSubstitutionSheet(
      orderId: 7,
      ...
      batchLoader: (catalogId) async => [
        {'inventory_item_id': 55, 'inventory_batch_id': 900, 'batch_number': 'B-NEAR', 'remaining_quantity': 50, 'expiry_date': '2027-01-01'},
      ],
      dispenser: ({
        required patientUid,
        encounterId,
    ...
    expect(find.text('Augmentin 625'), findsWidgets); // prescribed line surfaced
    ...
    final dispenseBtn = find.widgetWithText(FilledButton, 'Dispense substitute');
  ...
  testWidgets('no prescribed catalog lines → shows the cannot-resolve message', (tester) async {
    await tester.pumpWidget(_host(DispenseSubstitutionSheet(
      orderId: 8,
      contextLoader: (id) async => {'order_id': id, 'patient_uid': 'p', 'lines': const []},
    )));
    await tester.pumpAndSettle();
    expect(find.textContaining('No prescribed catalog lines'), findsOneWidget);
  });
}

...
| INF-005 | MinIO comments claimed four pools/16 pods while the manifest defined one pool/four servers. | `infra/kubernetes/base/minio/tenant.yaml` and the hardware doc reconciled to one authoritative topology. **Whole-node failure drill remains unproven — see ENV-2.** |
...
| OPEN-6 | **INF-006 dual release authority is unresolved.** `scripts/ci/README.md` still states "Forgejo is the canonical CI/CD target" while Argo watches GitHub and the handoff makes GitHub the only publication target. Both remotes retain code paths that can write production digests. | Owner selects one authority; the other becomes a verified one-way mirror with SHA-parity enforcement. Owner-deferred for this continuation (see DEFER-1) but the contradiction is live in the docs today. |
...
| ENV-2 | **MinIO tolerates whole-node failure at the reconciled topology.** The manifest and hardware doc now agree on one pool / four servers. | Parity and anti-affinity behaviour under real node loss cannot be inferred from a Kustomize render. | Whole-node failure drill against a real cluster. |

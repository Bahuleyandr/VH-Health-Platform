...
`9e70d950d` (PR
[#982](https://github.com/Bahuleyandr/VH-Health-Platform/pull/982)). Earlier the
...
| OPEN-14 | **The comprehensive seed's `checkedValue()` is not column-bound, and ordering the query made it deterministic without making it correct.** `seed-comprehensive-test-data.mjs:551-581` (sole caller `:1547`) picks a seed value for a text column by scanning that table's CHECK definitions: the column gate at `:573` is a bare `includes()` substring test (so `status` matches a definition mentioning only `order_status`, while the `formatConstrained` regex two lines above correctly uses `\b`), literals are harvested from the WHOLE definition (`:575`), and the FIRST survivor is returned (`:579`) regardless of which conjunct owns it. **Correcting the previous text in this row:** the failure mode is no longer unordered catalog iteration. `dce625f48` (2026-08-30, *"make the comprehensive seed deterministic"*) added `ORDER BY conrelid::regclass::text, conname` to the CHECK query (`:491`) after CI failed where a scratch DB had passed on `pharmacy_funding_decision_events` — *"same schema, same code, different result"*. That froze ONE arbitrary answer per column instead of fixing the selection: with definitions reversed, 199 text columns on the mig-762 schema answer differently, and ordering by `conname` means **renaming a constraint can change a seeded value**. The three `TABLE_COLUMN_SEED_OVERRIDES` pins (`:742`) exist to override frozen WRONG answers — `facility_asset_events.event_type` freezes to `status_changed`, the failing branch, which is precisely why it is pinned. Measured scope: **1,483 of 5,654 text columns** draw a value from this function, **968** from a multi-column CHECK and **65** via a substring collision on another column's name (e.g. `bulk_revision_jobs.hr_signature_sha256 = 'building'`, `diagnostic_result_generations.classification = 'object'` from a `jsonb_typeof` test, `clinical_ai_trial_sync_runs.source` set to a `LIKE` pattern). Still no test exercises `checkedValue`, which is why all three instances were found in CI rather than by a unit test. | Replace the heuristic with column-bound extraction that binds each literal to the conjunct constraining the requested column, tie-broken on definition TEXT rather than `conname` so a rename cannot move a seed value, and assert order-independence directly (both definition orders, not repeated runs — the order is stable today, so reruns prove nothing). Add the missing unit test over synthetic multi-column definitions and mutation-test it. Retire only the pins whose values the corrected function derives from the definitions in every order. Expected delta on NOT NULL generic-walker columns: 716 unchanged, 170 changed literal, 113 falling through to `semanticValue` — verify per column that those 113 satisfy every CHECK referencing them, including `NOT VALID` ones, which Postgres enforces on new rows. Do not loosen a constraint or paper over a rejected row; a row that passes today only because its column is otherwise unconstrained is passing by accident. |
| OPEN-16 | **Engagement-campaign approval is not materially version-bound.** The campaign insert and the approve/materialize transitions still bind no immutable content hash or version (`engagementCampaignService.js:523-543,955-976,997-1065`). `approveCampaign` (`:1016-1065`) enforces the governed role set, a distinct authenticated approver, a present submitter identity and a non-empty reason — and nothing about *what* was approved; the transition UPDATE stamps `approved_by`/`approved_at` (`:960-961`) without recording the approved material. Three consequences are live on current main. (a) `materializeCampaignRecipients` accepts post-approval statuses — `:864` allows `scheduled` and `running` — and overwrites `frozen_audience_hash` with a fresh cohort hash under the standing approval (`:921-932`), reachable at `routes/engagement/engagementRoutes.js:133`. (b) `frozen_audience_hash` is written but never compared anywhere in the backend: the only sites are the DDL (`migrations/436_engagement_campaigns_audiences.sql:28`), `schema.prisma:10480`, two SELECT lists (`engagementListQueries.js:128`, `engagementCampaignService.js:468`), the SET at `:924` and the RETURNING at `:969`. (c) `queueDueCampaignRecipients` selects on `campaign_id`/`status='eligible'`/`due_at` only, not on `current_audience_snapshot_id` (`:1076-1090`), so recipients materialized after approval are dispatched under the earlier approval. Coverage proves submitter/approver separation only. No commit between `a4ffe9860` and `61e7dcf7a` touched `apps/backend/src/services/engagement/`. | Persist a canonical material hash/version at submit, bind it into the approval row, and re-compare it at materialize and at queue time; expire the approval and return a materially edited campaign to draft so no change of template, audience, channel, schedule or rate policy survives an existing approval. Gate the post-approval materialize path and filter the due-recipient query by the approved snapshot. ROADMAP `:75-77` and `:1266-1282`. |
...
| OPEN-23 | **Inline CHECK constraints declared inside `CREATE TABLE IF NOT EXISTS` re-declarations of baseline-owned tables have never existed in any database.** `000_baseline.sql` creates the table first (files apply sorted by name, `ci-setup-db.mjs:14-23`), so the later migration's column-level CHECKs are silently discarded. Migration 155 is the reference case: `155_maternity_workflow.sql:39` re-declares `maternity_pregnancies`, whose CHECKs on `edd_method` (`:46`), `booking_status` (`:54`) and `status` (`:59`) are absent from `000_baseline.sql:12036-12060` — which carries only `maternity_pregnancies_pkey` while preserving 65 `CONSTRAINT ... CHECK` lines for other tables. Six live databases report `count(*)=0` for `contype='c'` on that table, and no migration ever adds them by `ALTER`. Root cause: `000_baseline.sql:2-8` records that the baseline replaced a `prisma db push` bootstrap; `maternity_pregnancies` is a Prisma model and Prisma cannot express CHECK constraints, so the checkless shape was pushed, `IF NOT EXISTS` no-op'd against it, and `pg_dump` froze it into the baseline — self-perpetuating on every regeneration. Repo-wide this is a class, not an instance: **369 CHECK clauses across 157 tables and 82 migration files are absent from the live schema** (42 pairs are satisfied and 8 partial, which calibrates the scan). Migration 155 loses 17 across all 8 of its tables; other affected tables include `dialysis_patients` (6), `mfa_devices` (5), `abdm_consent_requests` (5), `emergency_visits` (4), `postop_complication_alerts` (6). No gate can detect this — `check-schema-drift.mjs` has no `pg_constraint`/`contype` awareness and compares against a schema format that cannot represent CHECKs. Mostly defence-in-depth loss (e.g. `recordApgar` mirrors all six Apgar guards at `maternityService.js:3340-3348`), but `edd_method` and `booking_status` reach the INSERT caller-controlled with no enum guard at any layer (`maternityService.js:333,335,374,377`), so those two columns accept arbitrary 20-char strings today. Seeded rows are currently in-domain, so this is latent rather than active corruption; `maternityService.js:345-351` records a past incident of exactly this class, where rows landed with a `status` value outside the declared domain that a live CHECK would have rejected with 23514. | Triage the 369-clause inventory to separate dead CHECKs already mirrored in application code from those that are the sole guard on a clinically or security-relevant column (`dialysis_patients` serology/modality enums, `mfa_devices`, `abdm_consent_requests` first). For the sole-guard set, backfill or quarantine out-of-domain rows, then add the constraints in newly allocated forward migrations — `check-migration-immutability.mjs` forbids editing applied files, so never edit 155 or `000_baseline.sql`. Add a CI gate that parses inline CHECKs from every migration and asserts each is present in `pg_constraint` on the bootstrapped database, so the class cannot silently reappear; regenerating the baseline alone does not fix it, because the baseline is dumped from a database that already lost the constraints. Escalate to High if triage finds any sole-guard CHECK on an activated clinical or consent surface. |
| OPEN-24 | **The authenticated route crawl reports only the FIRST broken dashboard route, so "Smoke E2E is red" has never indicated how many routes are actually broken.** `apps/admin/e2e/route-crawl.spec.ts:107-108` iterates every discovered route inside a single test (`for (const route of routes) { await test.step(...) }`), so a failing `expect` inside a step propagates and ends the run. Routes are crawled in sorted order, which makes the masking systematic rather than random: `/dashboard/mar` sorts before `/dashboard/pharmacy`, and fixing MAR in #982 immediately exposed a pharmacy failure that had been present and invisible behind it (run `33726802402` reported only `/dashboard/mar`; run `33763189623` on the very next main reported only `/dashboard/pharmacy`). Every past reading of this signal as one defect was therefore an artefact of the harness. | Collect per-route failures and assert once at the end, so one run enumerates every broken route instead of the alphabetically first. Until then, treat a green crawl as evidence only about routes up to the first failure, and expect each fix to reveal the next. This is a measurement defect, not a product one, but it has been silently capping the value of the whole smoke tier. |
| OPEN-25 | **`/dashboard/pharmacy` fails the route crawl with a 409 tenant-configuration precondition, previously masked by OPEN-11.** Exact-main run `33763189623` on `9e70d950d`: `GET /api/v1/pharmacy-orders/orders/sla` returns `409 PHARMACY_FACILITY_REQUIRED`, *"No active default pharmacy facility is configured for this tenant"*, on all three Playwright attempts. This is a different class from OPEN-11 — not an authority mismatch but an unmet configuration precondition — and it is the current sole cause of Smoke E2E being red. | Owner decision on which of two readings applies, because they imply different fixes. Either the smoke environment must seed an active default pharmacy facility for its tenant (a seeding/environment gap), or the page must treat "no pharmacy facility configured" as a legitimate empty state rather than firing a call that cannot succeed (a product gap). Do NOT add it to `EXPECTED_DARK_GATE_RESPONSES`: unlike the two 503 entries there, this is not a deliberate dark gate, and waiving it would hide a real precondition while turning the tier green. |
| OPEN-26 | **Backend deep suites flake on concurrency and teardown, so a red backend shard is not currently a trustworthy signal.** PR #982 — a four-file, ADMIN-ONLY change with zero backend files — needed THREE CI runs to land, failing twice on unrelated backend suites and passing the third time with no code change. (1) Run `33754184612`, shard 3/3, `document-integrity.deep.test.js` failed to run: `23503`, `DELETE FROM tenants` blocked by `fk_audit_log_tenant`. Its teardown ordering is in fact correct — it deletes `audit_log` (`:62`) and `audit_logs` (`:67`) before `tenants` (`:85`) — so the cause is a late asynchronous audit write landing between the two, recreating the FK child. The suite already anticipates exactly this class for a different table: `waitForPhiAuditWrites` exists because *"phiAccessLogger writes hipaa_access_log rows AFTER the response"*. It defends one table and not the other. (2) Run `33759877082`, shard 1/3, `patient-data-import-vitals.deep.test.js` R8 *"atomically claims concurrent replays"* failed with `40001` (could not serialize) and `23505` (duplicate key on `ux_fhir_vital_obser...`), both under `async Promise.all`. Whether that test is racy or is exposing a service path that should retry `40001` and does not is NOT yet established, and the two readings imply different fixes. Backend shard assignment is computed from the backend test list alone (`run-ci-jest.mjs --listTests`, pinned by `jestShardPartition.test.js`), so an admin-only diff leaves the partitions byte-identical and cannot be the cause. | Extend `waitForPhiAuditWrites`-style settling to `audit_logs` in the FK case. For R8, first decide test-racy versus product-racy by reading the service path, then either tolerate the legitimate alternative outcome or make the write idempotent/retrying. Then amplify rather than rerun: a green rerun proves nothing about a race. The cost here is not the wasted cycles but that a red shard currently trains everyone to rerun instead of read, which is how a real regression eventually gets waved through. |

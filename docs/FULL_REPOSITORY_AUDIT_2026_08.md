...
| INF-008 | Dalekdefender deploy exited 0 when prerequisites were missing, so green could mean no deployment. | `.forgejo/workflows/deploy-dalekdefender.yml` and `scripts/forgejo-deploy-preflight.mjs` fail closed in required mode and emit machine-readable `not_deployed` otherwise. This is statically tested; a real Forgejo execution remains unproven under OPEN-7/ENV evidence. |
...
| OPEN-21 | **Five-locale key parity does not cover every user-visible English literal.** Current main and the locally verified but unpushed Malayalam candidate (`30903b6c8`, `49ca4c729`) still contain appointment errors/form labels/date copy (`apps/patient/lib/features/appointments/widgets/appointment_book_tab.dart:156,527,558,573`), About contact-action titles (`apps/patient/lib/features/about/screens/about_us_screen.dart:147,158,170`), dependent-setup errors, guardianship explanation, and consent (`apps/patient/lib/features/family/screens/family_screen.dart:726,737,778,833`), plus the held Staff Web title/message (`apps/staff/lib/main.dart:457,475`). The candidate's retained local receipt reports patient 31/31, Staff 54/54, and backend locale 57/57 tests green plus both Flutter analyzers at exit 0; that proves its structural/key-parity scope, not global presentation parity. Neither candidate commit is on `github/main`. | Route the appointment/About literals through all-five-locale resources and retain render/structural coverage. The dependent guardianship and consent wording remains fail-closed on legal and human linguistic review; technical placeholders are not approval. Staff Web activation copy requires operator/release ownership and must not be used to activate the held surface. |
...
| HELD-11 | **Alertmanager delivery activation.** Validation and route assertions are wired by #961, but no repository rule proves a real person receives a notification. | `.github/workflows/ci.yml:105-122`; `scripts/validate-alertmanager.mjs:63-72`; `infra/kubernetes/base/monitoring/alertmanager.yaml.example`. | Owner-supplied Discord/PagerDuty/Slack/SMTP values and team destinations; operator sealing and Kustomization inclusion; manual Argo sync; captured end-to-end delivery proof and rollback evidence. No real secret belongs in this ledger. |
| HELD-12 | **Human linguistic approval.** The five-locale technical contract is `en`/`hi`/`ta`/`te`/`ml`; #965 closes the known ABDM Malayalam omission. Technical key parity and placeholder wording are not approval of clinical, dosage, consent, legal, or ABDM meaning. OPEN-21 separately records the residual hardcoded presentation paths, including dependent guardianship/consent text that no technical lane may approve. | Locale resources, structural parity gates, `docs/TRANSLATION_REVIEW_TRACKER.md`, and the exact OPEN-21 patient paths. | Named human linguistic reviewers, with clinical/legal review where the string carries those meanings. |

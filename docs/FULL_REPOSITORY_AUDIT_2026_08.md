...
ratings and branch measurements remain attached to that snapshot.
...
[PR #1088](https://github.com/Bahuleyandr/VH-Health-Platform/pull/1088)
merged as `d60f1ea2c853a9fa525f4a15e60b9fe7b044fc4e` on September 16 UTC.
Migration `801_restore_maternity_labor_delivery_checks.sql` restores five
named CHECKs originally declared in migration 155 but absent from the
baseline-owned tables:

| Table | Column(s) | Verification boundary |
| --- | --- | --- |
| `maternity_labor_admissions` | `admission_reason`, `status` | Two named CHECKs; validation requires each live pre-check to find zero violating rows. |
| `maternity_partograph_entries` | `descent_fifths_above_brim`, `contractions_intensity` | Two named CHECKs; validation requires each live pre-check to find zero violating rows. |
| `maternity_deliveries` | `delivery_mode` | Named CHECK enforces new writes; historical validation remains explicitly deferred (`NOT VALID`). |

The seeded local verification recorded four validated CHECKs and the delivery
CHECK `NOT VALID`. Historical violations are not rewritten or silently accepted;
delivery validation still requires a clean pre-check and clinical-records/operator
review. Migration `802_widen_maternity_labor_status.sql` repairs the baseline
`VARCHAR(20)` width to 22 so the already-declared `discharged_undelivered` value
is representable; wider existing lineages are preserved. Neither migration
authorizes a clinical lifecycle transition. The database-backed regression is
`apps/backend/src/tests/maternity-labor-delivery-check-restoration-migration.deep.test.js`.

At final PR head `f25220b1b7ef7c8b6fd7809e273eb0cdfc60c5e9`,
[Canonical CI 35100279773](https://github.com/Bahuleyandr/VH-Health-Platform/actions/runs/35100279773)
succeeded with all three backend shards, six `full_*` jobs, and exact named
`Merge Gate` and `Full Merge Gate` contexts. Independent review accepted the
local lineage/runtime-role checks and full backend corpus (341 chunks, 2,042
suites, 23,685 tests, zero skipped) at source head
`de7c4b7610217a541ba6a8968debd831db7b5ed3`; the final child changes only the
migration-registry status cell and has its own security/contracts verification.
These are the implementation lane's receipts, not tests rerun by this
documentation change. Retained evidence includes
`D:/Dev/_codex/artifacts/logs/2026-09-16/maternity-802-successor-final-evidence.json`
and `pr1088-f252-independent-hosted-verdict.json` in the same operator directory.

Recomputed from the committed `scripts/ci/inline-check-census.json`: **465
declarations, 62 enforced and 403 absent across 178 tables and 86 declaring
files**. #1088 changes exactly five enforced flags; declaration identities and
the calibration logic are unchanged. Combined with #1079, eight maternity
CHECKs are restored. This documentation pass does not remeasure a database;
declaration counts are not distinct physical constraints or a risk ranking.

**OPEN-23 and maternity workflow closure remain open.** This receipt does not
claim end-to-end antenatal, delivery, newborn or postnatal client wiring,
historical-record acceptance, human linguistic approval, production readiness,
or postdeployment acceptance for #1088. No migration 803 claim, pending RLS
disposition or external-authority stop is changed.

### 2026-09-16 historical OPEN-23 receipt — migration 800
...
At the #1079 snapshot, `scripts/ci/inline-check-census.json` contains 465 declarations:
...
| Finding or lane | Current-main evidence | Disposition / boundary |
| --- | --- | --- |
...
All PRs #1013, #1014 and #1025–#1031 report `MERGED` and both named
`Merge Gate` / `Full Merge Gate` checks report success on their final PR
heads. For #1031, [Canonical run 34151295087](https://github.com/Bahuleyandr/VH-Health-Platform/actions/runs/34151295087)
ran the three backend shards, backend static checks, Admin, both Flutter jobs,
contracts, FHIR, gateway and infra successfully at `1eb2e93b9`; those jobs
were not tier-skipped.

The latest completed [Smoke E2E run 34094984786](https://github.com/Bahuleyandr/VH-Health-Platform/actions/runs/34094984786)
is green at `3d091a510`; [Full Stack Sweep 34089787452](https://github.com/Bahuleyandr/VH-Health-Platform/actions/runs/34089787452)
is green at `db30fe80b`. Both precede the frozen main tip, so neither is
exact-tip runtime evidence. No operator, linguistic, clinical, finance, legal,
vendor, partner, secret or release stop is cleared by these merges.

### 2026-09-03 historical current-main receipt

The following receipt describes its recorded September 3 heads only. Its
then-current PR list and red smoke results are superseded by the bounded
September 8 receipt above; its run IDs remain historical evidence.
...
Active rows below retain their latest stated evidence and severity. Only the
named September 8 delta was rechecked; the closed subsection preserves original
IDs without leaving them falsely open.
...
| OPEN-25 | **Verified implemented for both affected pharmacy tabs (PR #992 `25c8c99f9`, #996 `31be6be31`).** The earlier smoke-only facility seed removed the tenant-level 409 but exposed an actor-level 403. Overview and Orders now query the same facility-authority probe before requesting scoped data and render an honest unassigned state when the actor has no grant. Probe failure remains an error; an unresolved probe is not authority. | `apps/admin/src/app/(with-auth)/dashboard/pharmacy/components/useFacilityAuthority.ts:42-75`, `OverviewTab.tsx:33-75`, `OrdersTab.tsx:78-115,229-241`; regression cases in `apps/admin/src/__tests__/dashboard/pharmacy-facility-authority-gate.test.tsx:80-129` and `pharmacy-orders-facility-authority.test.tsx:58-115`. Both PR heads have green named merge gates; later Smoke E2E run `34094984786` at `3d091a510` is green. This does not grant facility authority, waive a 403, or prove every pharmacy workflow. |
| OPEN-26 | **Verified implemented for both recorded CI races (#986 and #1026 `3d091a510`, source `353d727ca`).** Document-integrity teardown retains bounded settling for late audit children (`apps/backend/src/tests/document-integrity.deep.test.js:48-81,186-188`). The import race is now diagnosed: PostgreSQL `40001` wrapped under `meta.driverAdapterError.cause.code` was masked by top-level/meta `P2010`, escaping an already-existing retry loop. The classifier now prefers the nested SQLSTATE and retries the complete SERIALIZABLE attempt, at most three times. | `apps/backend/src/services/import/patientDataImport.js:50-69,928,1003-1019`; focused carrier regression `apps/backend/src/tests/unit/patientDataImportSerializationRetry.test.js:1-20`; real-Postgres two-party barrier and retry-positive-control at `apps/backend/src/tests/patient-data-import-vitals.deep.test.js:2965-3055`. #1026 records 50 forced-conflict replay pairs, 50 retries and zero rejected/invalid outcomes, plus 67 deep tests and 49 related unit tests; these are the PR's measurements, not new local runs. Both final PR-head merge gates are green. Do not generalize this closure into permission to rerun unrelated failures until green. |
...
| OPEN-12 | **The migration-753 obligation for new-stock cath consumable usage remains an owner decision.** The earlier finding covered every nonterminal usage. PR #1004 (`a7dbcd03c`) narrowed it: `apps/backend/src/migrations/765_cath_device_reuse.sql:32-49,67-69` re-declares the contract with an exact `reused_device` branch that forbids stock movement and shortfall task/SLA/outbox artifacts for reused devices. Its header explicitly leaves ballot `753-D1` unresolved for **new units only**. The reused-device carve-out is implemented; it does not settle the ordinary-stock obligation. | Clinical/pharmacy/finance owners must decide whether every new-unit usage intentionally carries that obligation. The two options in `GO_LIVE_READINESS_GAP_MATRIX.md:44-66` remain blank, and activation remains stopped pending that decision and independent verification. Any repair must use a newly allocated forward migration, never edit 753, 758 or 765. Plan 1 blood-borne markers (#1000), cath device reuse (#1004), and the newer Plan 4 design/schema/kernel slices are merged; none is a substitute for this ballot. |

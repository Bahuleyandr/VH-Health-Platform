...
`61e7dcf7a3799227d1ac68010eb16041c1c0a446` (PR
[#980](https://github.com/Bahuleyandr/VH-Health-Platform/pull/980)). The previous
reconciliation was 2026-09-02 at `a4ffe9860596f07ae984d9334fe78f008b75907b`
(merge-train PR [#967](https://github.com/Bahuleyandr/VH-Health-Platform/pull/967)).
...
| OPEN-11 | **Still open on `61e7dcf7a`. Admin Smoke E2E is red because the MAR page authority is broader than its backend reads.** `/dashboard/mar` is reachable through the Admin role policy (`routePolicy.ts:164` — `mar: { minRank: STAFF }`; `routePolicy.ts:296-305`, where `roleSatisfiesPolicy` short-circuits `SUPER_ADMIN` to `true` at `:303`; `navConfig.ts:214`), and the page still fires both due-list reads on load (`apps/admin/src/app/(with-auth)/dashboard/mar/page.tsx:136,146`), but the backend correctly rejects the crawl's SUPER_ADMIN identity because it admits only inpatient nursing roles (`clinicalRoutes.js:145-153,186-191`, guarding `:877` `/mar/overdue` and `:899` `/mar/due`). The crawl carries no waiver for this route — `apps/admin/e2e/route-crawl.spec.ts:49-73` lists only the two 503 dark gates. The newest Smoke E2E run is `33726802402` (2026-09-03, `1c55ee04c`), still `failure`, reproducing both 403s (`Only inpatient nursing roles can enumerate due medications`) on all three Playwright attempts; run `33601968478` (2026-09-02, `a4ffe9860`) is the prior identical red. The workflow has not yet run against tip `61e7dcf7a`, but every anchor above is unchanged at tip, so the defect stands. | Preserve the backend nursing restriction. Align Admin route/navigation/page construction with the due-list role contract, and retain a test proving unauthorized identities cannot construct the page while an authorized nursing identity completes the route smoke. |
...
| OPEN-14 | **The comprehensive seed's `checkedValue()` remains a non-column-bound literal heuristic.** The format guard (`seed-comprehensive-test-data.mjs:567-573`) only skips a definition in which *this* column is itself regex/`LIKE`-constrained; any surviving definition is then scanned whole and the **first** literal in it is returned regardless of which conjunct owns it (`:574,:578`), with the sole column binding a bare substring test (`:572`, not word-boundary). So a `LIKE`-constrained or enum-adjacent column still receives a neighbouring conjunct's literal whenever its format constraint and its multi-column CHECK are separate `pg_constraint` rows. The function is now at `:551-581`. This is not hypothetical: the script itself records the resulting catalog-order-dependent seed flake — "it passed on one database and failed on another with the same schema" (`:776-780`) and "the intermittent 801/802 seeded-coverage failure" (`:1048-1054`). Current containment is the hand-maintained pinned-literal table `TABLE_COLUMN_SEED_OVERRIDES` (`:742`, consulted ahead of `checkedValue` at `:1541-1547`), i.e. per-column papering-over that must grow with every new multi-column CHECK. No test exercises `checkedValue()`. Unchanged in substance since `a4ffe9860`; the only commit touching this file in the wave, `b90f2bb06`, merely shifts the function +22 lines. | Replace the heuristic with column-bound CHECK-expression handling (associate each literal with the column its conjunct actually constrains), retire the override pins it exists to compensate for, add a unit test over synthetic multi-column CHECK definitions in both `pg_constraint` orders, then run the full deterministic seed/contract matrix. Do not loosen constraints or paper over rejected rows. |
...
| OPEN-22 | **Backend (and admin) dependency compatibility still relies on install-time mutation, and the mutation surface grew this wave.** Current main still globally overrides `minimatch` to 10.2.5 and runs `apps/backend/scripts/patch-minimatch-compat.mjs` from `postinstall`, copies that script into both backend image install stages, and gates the whole arrangement (`apps/backend/package.json:9,384,394`; `apps/backend/Dockerfile:8,52`; `scripts/security/check-infra-security-controls.mjs:21,25,40-41,102-110`). The identical arrangement exists in admin (`apps/admin/package.json:7,97`), and the gate at `:105` requires the two patch scripts to be byte-identical, so remediation must move both apps together. Since the 2026-09-02 reconciliation, commit `879f0194e` widened the hook from one vendored package to two: it now also rewrites `node_modules/test-exclude` pinned at 6.0.0 (`patch-minimatch-compat.mjs:7,129-131,174`) and became `--omit=dev`-aware for the runtime stage (`:13`). The 222-line script throws on any lock, version or source-anchor drift (`:34,:117,:126,:131,:172`), so a routine bump of *either* package hard-fails `npm ci` across both apps and both Docker stages. Normal `npm ci` still succeeds today because the hook runs, so this remains hardening debt rather than a live install outage — but the debt is now larger and more brittle than when this row was written. A single mutation-side test landed on main (`minimatchWindowsCoverageCompatibility.test.js`); it exercises the patch, not native resolution, so it does not advance closure. Local commit `e5699a180` removes the backend override/hook, but it is not an ancestor of `github/main` and appears on no remote branch, so it is not current-main completion. | Land an independently reviewed native-resolution change covering **both** `minimatch` and the newly added `test-exclude` patch, in **both** backend and admin, with fresh `npm ci`, direct ESLint/coverage compatibility imports, mutation-tested dependency-floor coverage, full backend lint/security including `check-infra-security-controls.mjs`, and both backend Docker install stages green. The retained local candidate `e5699a180` predates the `test-exclude` expansion and the admin byte-identity gate, so it requires re-scoping as well as fresh-main reconciliation. |

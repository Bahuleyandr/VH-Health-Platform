...
- [x] **M15** Forgejo pipelines: Trivy image+fs scans now blocking (`--exit-code 1`) incl. secrets; scanner images digest-pinned / installer refs version-pinned. Forgejo now also owns standalone secret scan, dependency-risk, smoke E2E, full-stack sweep, and warehouse/dbt gates. OSV stays advisory (backlog triage first).

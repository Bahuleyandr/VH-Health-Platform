...
- ⏳ **Remaining backend = refactors/epics (own design passes):** **M12** process-global breaker → per-tenant/observable, **M17** SSRF one shared `resilientFetch`, **M20** admin god-router split, **M15** make `decisionSupportOnly`/`patientFacing` load-bearing `[S→T2]`.

...
- ⏳ **Remaining backend = refactors (own design passes):** **M20** admin god-router split (mechanical, low-risk), **M12** process-global breaker → per-tenant/observable (hot path), **M15** make `decisionSupportOnly`/`patientFacing` load-bearing `[S→T2]`.

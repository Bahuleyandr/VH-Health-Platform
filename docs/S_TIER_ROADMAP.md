...
_2026-06-14 (WS4 B4.2 COMPLETE — BCMA server-side 2-scan enforcement) — Recon found B4.2 ~80% pre-built (scan-first endpoint, scan columns, override-with-reason, canonical audit, Flutter dual-scan payload, pharmacist clinical-verify — all present). Closed the gap: **migration 309** adds `patient_scanned_at`/`medication_scanned_at` (auditable 2-scan gate); `marFiveRightsService.administerWithScan` now enforces an explicit **`MAR_TWO_SCAN_REQUIRED`** gate (both patient + drug scans must match, else override=reason) BEFORE the bundled 5-rights check, records scan timestamps, and audits overrides (`two_scan_override` + timestamps in the canonical event); BOTH MAR write paths (administerWithScan + marService.recordAdministration) now run inside `setTenantTx` with `tenant_id` re-asserted in the WHERE, tenantId threaded from `req.tenantId`. Tests: clinical-safety (block/allow/override) + bcma-closed-loop HTTP. Validated against the non-superuser `qa_writer` role (RLS truly enforced): drift + lint:raw-params clean; **full chunked suite green 68/68 (6735 tests, 0 fail)**. WS4 now 6/6. Local-only._

_2026-06-14 (WS2 B2.5 COMPLETE — downtime LAN mirror + fallback metrics, REL-5) — Two parts: (1) named Prometheus counter `db_undefined_table_fallback_total` (+ warn) incremented at the prisma.js circuit-breaker choke point, scoped to EXACTLY 42P01 via the existing `extractSqlState` (reused), exposed on /metrics; the `recordUndefinedTableFallback()` incrementer avoids the prisma↔metrics import cycle. (2) DB-free static downtime mirror: the ward-pack generator best-effort writes each self-contained pack HTML to `DOWNTIME_MIRROR_DIR` (+ index.html); new route `GET /downtime/static[/wards/:id]` serves them from the FILESYSTEM (never imports prisma), mounted before validateApiKey/jwtAuth, token-gated via `requireProductionMonitoringAccess`, path-traversal-safe, missing→200 paper-fallback HTML — so packs stay reachable with the backend DB down. `DOWNTIME_MIRROR_DIR` registered warn-tier in validateEnv. Validated: eslint + lint:raw-params + drift clean; full chunked suite green 68/68 (6698 tests, 0 fail). WS2 now 8/8. **Operator-gated infra follow-up** (out of code scope): shared hostPath/Longhorn volume at DOWNTIME_MIRROR_DIR for the CronJob + Deployment + ops-box LAN sync. Local-only._

_2026-06-14 (WS3 B3.2 COMPLETE — backend coverage floors, full ≥80% per user choice) — Coverage was UNGATED. Raised all 5 named domains to ≥80% statements via ~20 new `*Coverage` unit suites (parallel agents, domain-by-domain): **billing** (cashDrawer 0→100, paymentLink 21→99, billingService 70→99.5, billingV2 33→98.6), **auth** (authService 36→99.7, staffAuth 23→100, firebaseAuth 37→100, adminOtp 9→100, otpService 53→100, apiClient 76→97, userActiveSession 50→100, jwtMiddleware 53→96), **cds** (cdsEngine 33→98), **prescription** (80→93.8, branches 67→84), **RLS** (prisma.js 43→97, tenantContext 71→100, tenantContextMiddleware 16→100). **2 genuine PRODUCTION BUGS found + fixed by the coverage work:** (1) `adminOtpService` — 4 callers treated the private query() `{rows,rowCount}` wrapper as the rows array → `forceSendOtp` 500'd on EVERY admin force-send (success branch dead); (2) `prescriptionSafetyCheck` — a medication line with no name matched EVERY recorded allergy (`allergyName.includes('')`) → a spurious severe-allergy HARD BLOCKER on legitimate prescriptions. Both fixed + regression-tested. GATE: package.json `collectCoverageFrom` (5 domains) + 23 per-file `coverageThreshold` (stmts/lines/fns ≥80; branches at the achievable floor) + `npm run test:coverage` — a curated 49-suite in-band runner (the chunked `test:ci` does NOT collect coverage; a naive full `jest --coverage` OOMs; enforcement empirically proven by a deliberately-too-high floor failing). OPEN-HANDLE (CI-8): `scheduler.js` cron registrations guarded by `NODE_ENV!=='test'` (no reproducing leak found; `--forceExit` retained). test:coverage green (49 suites/1051 tests, aggregate 97.3% stmts); **full chunked suite green 70/70 (7535 tests, 0 fail)**. WS3 now 5/5. Local-only._

_2026-06-14 (WS1 B1.2 COMPLETE — multi-tenant INSERT completion) — migration **310** ALTERs every policied `tenant_id` column (**379 tables**, DO-loop over the pg_policy tenant_isolation set) DEFAULT from the literal default tenant to `COALESCE(NULLIF(NULLIF(current_setting('app.current_tenant_id',true),''),'bypass')::uuid, <default-tenant>)`. So an INSERT under `setTenant(X)` that omits tenant_id now auto-populates tenant_id=X (was the literal default → 42501 under the enforced non-owner role for any non-default tenant); GUC unset/'bypass' → default tenant (single-tenant unchanged); explicit tenant_id untouched. **This closes the FULL multi-tenant RLS story:** the prisma proxy already auto-scopes single-statement raw + model-API calls under AUTH_ENFORCE_TENANT_RLS (`maybeRunUnderTenant`/`wrapModelDelegate`), Batch 3 scoped the interactive `$transaction` callbacks (the only proxy bypass), and 310 makes the default-relying INSERTs land in the right tenant — so the formerly-'deferred' single-statement-raw + cancelAppointment-else-branch items are NOT gaps (auto-handled under enforcement) and B0b's MISROUTE inserts are subsumed. schema.prisma regenerated (379 `@default(dbgenerated(...))` — db pull captures the expression → drift GREEN). Regression test (tenant-rls.deep): insert under setTenant(TENANT_B) w/o tenant_id → tenant_id=TENANT_B under the non-bypassrls role; + direct psql proof (non-default/unset/bypass/explicit). Bonus: fixed a pre-existing `qa-cluster-up.mjs` role-provisioning gap (rls_journey_test_app not granted to qa_writer → cross-tenant-rls.journey failed after a role reset). **Full chunked suite green 70/70 (7621 tests, 0 fail).** Local-only._

_2026-06-14 (WS5 B5.5 COMPLETE — clinical-AI knowledge curation; **🏁 S-TIER ROADMAP WS0–WS8 IN-OUR-CONTROL COMPLETE**) — Bridged the hospital's own structured sources into the pgvector RAG KB (mig 113): `knowledgeCurationService` imports formulary (`pharmacy_catalog`) / antibiogram (`antibiogram_90d` view) / protocols (`clinical_protocols`) → render → SHA-256 dedup → `createInlineDocument(curationStatus='pending')` → chunk+embed (degrades to `embed_unavailable` when Ollama down). Migration **311**: `curation_status`/`reviewed_by`/`reviewed_at` on knowledge_documents (imports land 'pending', dark to retrieval until approved; manual uploads stay 'approved') + `knowledge_import_batches` audit + flagged starter KBs. Curation gate: retrieval filters `curation_status='approved'`; `decideKnowledgeDocument` + PATCH route (decide* pattern). Weekly `knowledge-corpus-refresh` cron (inside the NODE_ENV-guarded block). CLI `scripts/knowledge-curation-import.mjs` + `docs/CLINICAL_AI_KNOWLEDGE_CURATION.md`. Data is hospital-owned (NOT a flagged vendor feed); live embedding needs the B5.4 Ollama tier (operator-gated). Verified: drift + lint:raw-params + check:phi-tenant-id clean; 128 KB tests green; CLI dry-run rendered 57 formulary docs; idempotency + decide-curation proven. **Full chunked suite green 70/70 (7516 tests, 0 fail).** WS5 now 5/5.

**🏁 S-TIER ROADMAP COMPLETE (in-our-control):** WS0 7/7 · WS1 7+B1.3b (FULL multi-tenant RLS) · WS2 8/8 · WS3 5/5 · WS4 6/6 · WS5 5/5 · WS6 5/5 · WS7 4/4 (cert-ready) · WS8 4/4. **Pushed to both remotes 2026-06-14** (`main` @ `25ec5b87` on Forgejo + GitHub); the push surfaced 6 CI failures a green local suite hid (clean-DB test FK, CodeQL `actions:read` perm, malformed-but-now-blocking semgrep, esbuild audit advisory, SEC-8 admin build-arg, 2 gitleaks FPs) — all fixed, **GHA CI 6/6 green** (see lessons.md + `docs/GO_LIVE_ACTIVATION_CHECKLIST.md`). Remaining = `[flagged]`/operator-gated only: licensed vendor feeds (drug-KB Medi-Span, full formulary), GPU node + ward pilot, external engagements (ABDM cert / NABH assessment / DPDP audit / pen-test), operator infra toggles (`AUTH_ENFORCE_TENANT_RLS` + non-owner runtime role, Kyverno Enforce, sealed secrets, DR drill, shared LAN-mirror volume). 2 production bugs fixed en route (adminOtp force-send 500; prescription allergy false-blocker). lessons.md updated. Local-only._

| WS | Batches | Done | Status |
|---|---|---|---|
| WS0 | 7 | 7 | ✅ COMPLETE |
| WS1 | 7 | 7 + B1.3b | core ✅ (RLS 283-tbl, crypto envelope, SEC-5/6/7/8, pen-prep). **B1.3b REOPENED 2026-06-14** — 96-agent adversarial audit of all 127 `$transaction` sites found **101 cross-tenant actions** (bare-tx skips the GUC → permissive RLS; + 13 INSERTs missing tenant_id), not the "~97 hardening" originally assumed. Done: Batch 0a (7 MISROUTE/tx fixes incl `markDelivered` cross-tenant write) ✅ · 149-file prisma-mock tenant-helper sweep ✅ (test-infra enabler) · claimCaps/patientIdentifier hardened ✅ · Batch 1 (44 service converts, `4538dc23`) ✅ · Batch 2+4 ✅ · **Batch 3 ✅ — all 47 remaining PHI `$transaction` sites converted/threaded in 6 adversarially-verified waves (incl `cdsEngine.acknowledgeAlert` cross-tenant fix; appointmentReaper superAdmin; 8 non-RLS correctly skipped).** With Batch 0a/1/2/4 this closes all ~101 audited cross-tenant actions → **B1.3b CORE COMPLETE**. · **B1.2 multi-tenant INSERT completion ✅** (mig 310: GUC-reading `tenant_id` defaults on all 379 policied tables → `setTenant(X)` inserts auto-scope to X; the proxy ALS wrapper already auto-scopes reads/model/single-statement-raw under enforcement, so the formerly-'deferred' single-statement + cancelAppointment items are auto-handled, not gaps; B0b MISROUTE subsumed) → **FULL multi-tenant RLS COMPLETE**. **Suite green 70/70 chunks / 7621 tests (2026-06-14), 11/11 journeys** |
| WS2 | 8 | 8 | ✅ COMPLETE — B2.1 monitoring ✅ · INF-3/9/10/11 ✅ · B2.4/B2.8 timeouts ✅ · B2.6 Longhorn ✅ · B2.7 Vault auto-unseal ✅ · B2.2 DR-drill harness ✅ (operator runs the timed drill) · B2.3 outage-critical CronJobs (downtime-pack/backup-verify/canary) + WardDowntimePacksStale fix ✅ · **B2.5 downtime LAN mirror + fallback metrics ✅** (named 42P01 counter at the prisma breaker choke point + DB-free token-gated `/downtime/static` mirror; suite 68/68; operator-gated: shared volume + LAN sync) |
| WS3 | 5 | 5 | ✅ COMPLETE — B3.1 deterministic journey gate (harness + **11/11 journeys**, blocking CI; incl cross-tenant-rls regression test) ✅ · B3.2 backend coverage floors ✅ (**all 5 domains ≥80%** via ~20 new *Coverage suites; per-file coverageThreshold + `test:coverage` runner; scheduler open-handle guard; **2 prod bugs found+fixed** — adminOtp force-send 500 + prescription allergy false-blocker; suite 70/70) · B3.3 Flutter clinical widget tests (132) ✅ · B3.4 FHIR conformance + SAST BLOCKING ✅ · B3.5 cosign verify + ci.yml consolidation ✅ |
| WS4 | 6 | 6 | ✅ COMPLETE — B4.1 drug KB + interaction/allergy/dose-range CDS (`drugKnowledgeBaseService.js` + mig 277; vendor import [flagged]) ✅ · B4.3 med-rec (mig 279 + structured change-detail/atomic safety-review, mig 308) ✅ · B4.4 Orthanc PACS + OHIF + ArgoCD ✅ · B4.5 problem-list + terminology (mig 306/307) ✅ · B4.6 audit hash-chain (mig 282) + tamper test ✅ · **B4.2 BCMA server-side 2-scan enforcement ✅** (mig 309 scan timestamps + explicit `MAR_TWO_SCAN_REQUIRED` gate + `setTenantTx` on both MAR write paths; 110 tests under real-RLS qa_writer; suite 68/68) |
| WS5 | 5 | 5 | ✅ COMPLETE — B5.1 AI safety holes ✅ · B5.2 triage governance (local-default/model-bump/defenses) ✅ · B5.3 outcome scoreboard ✅ · B5.4 deep-tier readiness (Ollama preflight Job + egress NetworkPolicy + readOnlyRootFs; GPU node + ward pilot flagged) ✅ · **B5.5 knowledge curation ✅** (formulary/antibiogram/protocol → RAG import + curation gate + weekly refresh + starter KBs; mig 311; data hospital-owned, live-embed needs B5.4 Ollama tier) |

...
8. **Discharge-summary incorporation (owner ask, 2026-07-11)** — wire cath reports into the discharge compose pipeline (`apps/backend/src/services/ai/dischargeComposeService.js` `getComposeGraph`/`composeDischargePackage`, mig-159 summary builder): when a discharge summary is composed for an admission whose encounters carry **signed** cath reports, auto-include a "Cath Lab Procedures" section — per report: report_type, procedure date, operator/signer, key coded fields (e.g. vessels treated / stents for PTCA, device model + lead parameters for PPI), the findings summary, and a reference to the full report (and viewer link where PACS is enabled). Rules: **signed reports only** (drafts/preliminary excluded — a discharge summary must never carry unsigned findings); the section is auto-populated but clinician-editable in the builder before discharge sign-off (same posture as other composed sections); if a cath procedure log exists WITHOUT a signed report, surface a "cath report pending" completeness warning in the builder rather than silently omitting it. Addenda signed after discharge do NOT retro-edit the issued summary (append-only doctrine).

## Tests
- Unit: template versioning/supersede; report lifecycle transitions (draft→preliminary→signed; post-sign edit rejected; addendum append-only); edit-gate matrix (receptionist can draft, cannot sign; technician cannot edit); sign privilege gate fail-closed when enabled; viewer-link role matrix + `pacs_not_configured` shape; discharge compose includes signed-only reports + pending-report warning when a procedure log lacks one.

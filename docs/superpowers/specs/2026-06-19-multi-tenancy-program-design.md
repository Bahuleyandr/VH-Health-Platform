...
- **Status:** 📐 DESIGNED (2026-06-21) — operator/infra wave; execution is operator-led (cluster + Cloudflare + signing/Firebase). Design `docs/superpowers/specs/2026-06-21-w7-infra-onboarding-design.md` + runbook `docs/TENANT_ONBOARDING_RUNBOOK.md`. Key findings: `*.api.vhhealth.app` is a 2nd-level wildcard NOT covered by Cloudflare Universal SSL (needs ACM, or a flat `<slug>-api` structure); the cloudflared wildcard rule must PRESERVE the Host (`httpHostHeader: ""`) so W4's `parseTenantSlug` sees the slug; onboarding = an idempotent orchestrator wrapping createTenant + KEK + interop secrets + bootstrap-admin + preflight + per-tenant `--dart-define` builds + Phase-E RLS verify. 3 operator decisions: subdomain/TLS model, Firebase shared-vs-per-tenant, when to flip `ALLOW_DEFAULT_TENANT=false`.

...
- **Status:** ✅ COMPLETE (2026-06-21) — C1 `tenantFromHost`/`TENANT_BASE_HOST` · C2 `resolveTenantForRequest` Host-first · C3 lock-in (no client-header trust) · C4 `tenantContextMiddleware` Host↔token cross-check (`TENANT_HOST_TOKEN_MISMATCH`) · C5 every login path mints the right `tenant_id` (admins-realm fallback in both resolvers + claims on every bare mint; admin MFA mints resolve via raw SQL since the Prisma `admins` client was stale on `tenant_id`). Full chunked-as-postgres gate GREEN ("All chunks passed", 691 files / 87 chunks). No migration in W4. Detail in `2026-06-20-w4-edge-routing-token-tenant-claim-design.md` (Status line) + the memory. Wildcard subdomain DNS/TLS = operator/HELD (W7).
...
- **Status:** ✅ COMPLETE (2026-06-21) — S1 backend `GET /admin/tenant-context` (own tenant + branding) · S2 admin `TenantProvider` + branded chrome (`--tenant-primary`) · S3 SUPER_ADMIN act-as-tenant console (server-trusted `/api/act-as` httpOnly cookie + banner + `/dashboard/tenants` action) · S4 proxy strips client tenant headers, re-attaches only for a signature-verified SUPER_ADMIN acting-as (double-gated with the backend's audited override). Mostly client-side — W4 already shipped the backend (admin token tenant_id, audited override, tenant CRUD, routePolicy gating). Backend chunked gate GREEN + admin test/tsc/eslint/build green. Spec/plan `2026-06-21-w5-*`. NO-OP for the single default tenant.
...
- **Status:** 🛠 DESIGNED + SCAFFOLDED (2026-06-21) — code/manifests written + locally verified; only operator EXECUTION remains. Design `2026-06-21-w7-infra-onboarding-design.md` + runbook `docs/TENANT_ONBOARDING_RUNBOOK.md`. Built: Part-A wildcard manifests (cloudflared + backend/admin ingress, `17ac5868`, kustomize-validated, INERT until DNS/ACM — `*.api` is a 2nd-level wildcard NOT in Cloudflare Universal SSL → needs ACM; the cloudflared wildcard rule omits `httpHostHeader` so the Host is preserved for `parseTenantSlug`); Part-B idempotent `onboard-tenant.mjs` orchestrator (`3fe156d8`, QA-DB-verified: tenant+branding+KEK+admin); build helper `scripts/build-tenant-client.sh` (`8176854f`). Firebase decided: shared now → per-tenant later. Remaining = operator: apply manifests, Cloudflare DNS+ACM, wire Gradle flavors/signing/Firebase, run the orchestrator, flip `ALLOW_DEFAULT_TENANT=false` after cutover.

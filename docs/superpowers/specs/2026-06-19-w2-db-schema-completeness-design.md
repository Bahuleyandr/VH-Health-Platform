...
- **Status:** ✅ COMPLETE + gated (2026-06-19). Migrations 328–336 applied; comprehensive 2-tenant deep suite (`tenant-rls-w2-schema.deep.test.js`, 11/11) added; full chunked-as-postgres gate GREEN on a rebuilt QA DB ("All chunks passed", 679 files/85 chunks). Final state: 502 tenant_id tables all policied (`check:phi-tenant-id` allowlist empty), `check-schema-drift` clean, `check:no-default-tenant-fallback` clean; residual tenant_id-less = exactly 34 global-by-design tables. Commits 329 `6a3622fe` · 330 `50106c0b` · 331 `225b5a2a` · 332 `3a0518bc` · 333 `e5528bf1` · 334 `f7837afd` · 335 `8751976e` · 336 `46a91e99` · suite `0900986d` · ON CONFLICT code fixes `84af9dc6`. See the memory `project_vh_health_multitenancy_program` for the reusable W2 learnings (ON-CONFLICT-after-unique-swap, type-not-name backfill, static phi-guard ARRAY-only harvest, append-only audit_bypass, rebuild-needs-6-extensions). Deviations from this design, all documented in-commit: §330 folded `salary_revisions.revision_number` in (so §336 no longer handles it); §333 left `users.email` non-unique (no prior contract + legitimately shared); §334 chose an asymmetric tenant_isolation policy over a POLICY_ALLOWLIST exemption; §336 left investigations/pharmacy barcodes + `devices.device_id` global (unique-by-construction) and resolved 8 ambiguous tables (hospitals/blood_banks/pharmacies/medications/health_milestones/clinical_ai_guardrails/system_alerts global, discharge_summary_sections tenant-owned).

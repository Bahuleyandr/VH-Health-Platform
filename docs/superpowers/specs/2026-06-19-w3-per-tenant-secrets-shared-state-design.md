...
- **Status:** ✅ COMPLETE + gated (2026-06-20). All 7 workstreams implemented (TDD, per-workstream gated) + the full chunked-as-postgres gate GREEN on a rebuilt QA DB ("All chunks passed", 687 files/86 chunks, 0 failures). Migrations 337 (encryption_keys.wrapped_key_material) + 338 (tenant_interop_secrets); drift clean (538 models); check:phi-tenant-id 503/503 policied; no-default-fallback + raw-params clean. Commits: design `ee42bb8b` · plan `4bcbd6b6` · WS1 `80befc7c` · WS2 `04411425` · WS3 `ad4e131b` · WS4 `6869c2d0` · WS5 `e11e043a` · WS6 `e77e3213` · WS7 `ea9ad024` · gate-mock-fix `7d0b37e7`. See the program memory `project_vh_health_multitenancy_program` for the reusable W3 learnings. **Deviations / residuals (documented):** (a) WS5 per-tenant KEK covers the `fieldEncryption` (getKekProvider) subsystem; the mig-132 `kmsProviderService` PHI-shadow-column subsystem is a follow-on (same pattern, second provider). (b) WS6 default tenant is **env-backed in-route** (the route falls back to `ABDM_CONFIG.callbackSecret` / `HL7_INBOUND_SHARED_SECRET` when no per-tenant row), not a DB seed row — cleaner NO-OP for the existing tests; HL7 resolves tenant by the **MSH-6 receiving** facility (not sending). (c) WS3 cache left dormant/unmounted (decision C). (d) WS7 per-job query-scoping rides on the W1/W2 enforcement proxy (active at the W7 cutover). A separately-flagged PRE-EXISTING flaky concurrency test (`theatre-clinical-safety` anaesthesia-totals lost-update race) is NOT a W3 regression.

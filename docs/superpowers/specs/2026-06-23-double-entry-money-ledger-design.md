...
`AFTER INSERT` trigger on `ledger_postings` upserts the **normal-direction** delta into `ledger_balances(account, dimension)` (per the §3.4 sign convention), taking the `ledger_balances` row lock. A uniform CHECK `balance_paise ≥ 0` on the constrained accounts makes bad states uncommittable:

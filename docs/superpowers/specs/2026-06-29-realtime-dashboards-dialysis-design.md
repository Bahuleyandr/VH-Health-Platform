...
```
**Scope = `staff:` (isStaff) — a deliberate, bounded-safe SUPERSET of the REST audience (not an exact mirror).** The dialysis router has **two** gates: an app-level `requireRole(...DIALYSIS_ROUTE_ROLES)` at `app.js:1155` AND the in-router `requireStaffOrAdmin`. `DIALYSIS_ROUTE_ROLES = mergeRoles(getRolesForCapabilityGroups(['ip_flow','specialty_services']), ['DOCTOR'])` is a **capability-narrowed subset** of `isStaff`, so the effective REST audience is that subset — narrower than the `staff:` channel (full `isStaff`). The `staff:` channel therefore **over-grants**: staff roles outside `DIALYSIS_ROUTE_ROLES` (e.g. LAB_STAFF, PHARMACY_STAFF, HR_STAFF, HOUSEKEEPING, RECEPTIONIST, SECURITY) are 403'd on the dialysis REST API yet can subscribe. **This is bounded safe and accepted:** the WS payload is PHI-free `{kind, at}` (see §6), so an over-granted subscriber learns only activity-timing metadata ("a dialysis event happened at T"), never PHI, and any data refetch still goes through the capability-gated REST API. It is also the **systemic, pre-existing pattern of the whole epic** — `staff:lab` (`LAB_ROUTE_ROLES`), `staff:micro` (`MICROBIOLOGY_ROUTE_ROLES`), and `staff:icu-board` (`ICU_ROUTE_ROLES`) all pair a capability-narrowed REST gate with a `staff:` channel; the channel taxonomy (`staff:` / `staff:clinical:` / `admin:` / `patient:`) has no arbitrary-capability-subset scope, so `staff:` is the correct available choice and tightening would be an epic-wide taxonomy change, out of scope here. SUPER_ADMIN is admitted via the slice-9 `authorizeChannel` bypass. (Aside: `DIALYSIS_TECHNICIAN` appears only in the admin's frontend `routePolicy.ts` rank table, not in backend `ALL_STAFF_ROLES`/`roles.js`, so it is not an assignable backend role today — out of scope to reconcile.)
...
| Risk | Mitigation |
|---|---|
| Channel denies a real board user | `staff:` (isStaff) is a SUPERSET of the capability-narrowed `DIALYSIS_ROUTE_ROLES` REST audience, so every legitimate dialysis user can subscribe (no under-grant). Asserted in the channel test. |
| Channel over-grants vs REST (non-dialysis staff subscribe) | Accepted + bounded: PHI-free `{kind,at}` payload (only timing metadata leaks); data refetch stays capability-gated at the REST layer; systemic across `staff:lab`/`staff:micro`/`staff:icu-board`; no role-subset channel exists in the taxonomy. Documented in §4.1. |

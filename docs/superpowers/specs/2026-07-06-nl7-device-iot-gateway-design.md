...
- `apps/backend/src/services/auth/apiClientService.js` — `api_clients`/`api_keys` (tenant-scoped, scopes, allowed_ips, rate-limit profile, bcrypt key hash + prefix). **Dormant, and earmarked by NL-11 ("developer portal — activate `api_clients`"); NL-7 deliberately does not squat on it.**

...
- **Follow-up 3, now built:** the row set carries a **Facility** column and a **Time in queue** column, both sortable. `listDevices` joins `facilities` (tenant-pinned) for `facility_name` and derives `status_changed_at` — when the device last MOVED — from the newest transition row in its own audit trail. It is derived rather than read from a column because the register has none, and `updated_at` is not a substitute: `flagDeviceExposureTx` moves `updated_at` when the late-reactive sweep stamps a marker, which changes no status, so a device queued for a week would read as freshly touched on the day some OTHER patient's result came back. The per-status columns do not cover it either — `discarded_at`, `quarantined_at` and `last_reprocessed_at` exist, but `awaiting_reprocessing`, the status a queue is about, has none. `cath_device.exposure_flagged` is excluded from the derivation by name; the deep suite asserts the sweep moves `updated_at` and leaves `status_changed_at` alone. The queue's list item is published as its own `CssdDeviceQueueItem` schema (the device row plus those two), since `CathReprocessableDevice` is `additionalProperties:false` and every other device surface returns it unwidened.
...
- **As built, additional follow-ups recorded during implementation:**
  ...
  - ~~The Devices tab has no facility column and no time-in-queue column.~~ **Closed** — both added and sortable, with `status_changed_at` derived from the transition audit trail (§6.4).
  - **Queue read cost is linear in the tenant's device count.** The `status_changed_at` lateral runs one indexed `audit_logs` lookup per row returned (`idx_audit_logs_tenant_resource_time`, `LIMIT 1`), so a 200-row page is 200 index probes. That is the right shape at today's volumes and it does not degrade with audit-trail depth, but a tenant whose queue page grows or whose `audit_logs` partitioning changes should be measured before the page cap is raised: the alternative is a `status_changed_at` column on the register maintained by the transition writer, which trades this read cost for a write-path invariant.
  - **The RLS-blind harness.** The queue's `status_changed_at` is derived from a lateral join onto `audit_logs`, but nothing automated exercises that join under row-level security: the deep suite (`cath-device-reuse.deep.test.js`) and CI's `ci-setup-db` both connect as the database superuser, for whom every RLS policy is bypassed, so a policy on `audit_logs` that refused the reader would still let every test pass and would show up first in production as a queue where every row reads "just now". The join is tenant-pinned in SQL (`a.tenant_id = d.tenant_id`), which is what makes it correct rather than merely permitted, and the audit_logs RLS posture is covered separately against the live database — but there is no automated coverage of THIS read under a non-superuser role. Adding one means a harness that connects as the tenant runtime role, which is a change to the shared deep-test bootstrap and out of scope here.

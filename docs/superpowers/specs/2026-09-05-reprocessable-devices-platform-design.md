...
- Date: 2026-09-05
- Status: owner decisions D1–D9 taken in conversation on 2026-09-05; this document designs within them. Design review round 1 (2026-09-05) applied: the un-capture path for cancelled cases (G1), executor fixes 1–10 and the review follow-ups; one new owner item, D10 (§2, §9)
...
`bloodborneMarkerService.resolveReuseStatus({ tenantId, patientUid, validityDays, asOf, db })` is already domain-free (it is patient-scoped and reads only `patient_bloodborne_markers`). Both new domains call it with the domain's `serology_validity_days` from `reprocessing_domain_settings`. OT calls it directly (`instrument_sets` and `ot_schedules` carry no legacy serology). Dialysis never calls it directly on a patient path: capture, the reprocessing record and the session read all go through `dialysisReuseService.resolveDialysisRestriction`, the union read of §3.3, so the frozen `reuse_screen` / `post_use_screen` carry `legacy_source: true` when the restriction came from the enrolment columns rather than the marker record. The evidence-freezing rule from the cath spec §7.4 holds: the screen is stored as `reuse_screen` at capture and `post_use_screen` at return on `reprocessable_device_usages`, and a later result changes the device through the exposure handler, never the historical rows.
...
- `dialysisReuseService.syncDialysisPatientSerology({ tenantId, patientUid })` maps the resolver's latest per core marker to the legacy vocabulary — `reactive → 'positive'`, `non_reactive` within window → `'negative'`, `pending → 'pending'`, anything else or absent → `'unknown'` — with the latch rule intact (a latched reactive row yields `'positive'` whatever came after), and **never downgrades an existing `'positive'`** (a column set before Plan 1 existed, for a patient with no marker row, must not be flipped to `'unknown'` by a sync that knows less than the person who typed it). It is called from the exposure handler, from `recordSerology`, from the marker void path and from the enrolment path; the reconciliation sweep (§10) repeats it for every active roster row.
  - **The void path is wired, not assumed.** `voidMarker` notifies `registerMarkerVoidHandler` consumers post-commit — the exposure registry's shape (`bloodborneMarkerRules.js`), with the event `{ tenantId, patientUid, marker, result, markerRowId, voidedBy }` — and `dialysisReuseService` registers the sync at module load. A voided fresh non-reactive reverts `'negative' → 'unknown'` (the resolver no longer sees it); a voided reactive leaves `'positive'` standing — the no-downgrade latch holds on a void as on any other sync, so retracting a reactive row never un-restricts a roster row on its own (the union read in the last bullet keeps the patient restricted until the column is corrected by hand, §10).
  - **Negatives can lag.** The sync is event-driven on reactive rows and voids only: a fresh non-reactive recorded through the lab path pushes nothing (the exposure registry fires on reactive rows), so a `'negative'` column can lag until the next event or the reconciliation sweep. Cosmetic by construction — every restriction read consults the resolver, never the column.
...
- **The restriction read for dialysis is the union of the two records during transition.** `dialysisReuseService.resolveDialysisRestriction` returns the resolver output, and when the resolver says `clear` or `unknown` while a legacy column still says `'positive'`, it returns `restricted` with reason `'<marker> positive on dialysis enrolment record (pre-marker)'` and `legacy_source: true`. The legacy read is `bool_or(<column> = 'positive')` across **every** `dialysis_patients` row of the patient in the tenant, not the latest row: a patient re-enrolled after a transfer or a modality change has more than one roster row, and a `'positive'` typed on the older one must not be missed by an `ORDER BY id DESC LIMIT 1`. The union collapses to the resolver once the operator has run the backfill script (`scripts/backfill-dialysis-markers.mjs`, §7.6), which writes `clinical_declaration` rows from legacy `'positive'` columns under a named actor — a SQL backfill cannot supply `recorded_by`. Until then, a patient is restricted if *either* record says so; a patient is never made less restricted by this change.
  - **No correction path for a wrong legacy `'positive'`.** A typo on enrolment stays: the no-downgrade rule keeps the column, the union keeps the patient restricted, and the backfill turns it into a `clinical_declaration` marker. Correcting one needs a reviewed admin command that voids the backfilled marker and clears the column under a named actor with a reason; it is not in this lane (§10). Until it exists, the correction is the infection-control officer's manual row edit, audited as such.

### 3.4 The dialysis isolation WARN rule

Evaluated in `startSession` and `scheduleSession` (both existing) for the session's machine and patient, and re-evaluated by `PATCH /sessions/:id/machine` (new, §6.1). In `scheduleSession` the evaluation runs **before** the `INSERT`, inside one `setTenantTx` (today's `scheduleSession` inserts with the bare client outside any transaction — `dialysisService.js:466-481` — so a `block` refusal raised after the insert would leave an orphan `scheduled` row): `assessIsolationTx` reads and decides without writing, the `INSERT` itself carries the isolation columns, and a `DIALYSIS_ISOLATION_MACHINE_BLOCKED` at scheduling leaves no row (a deep test asserts the count). `startSession` and the reassign route call `evaluateIsolationForSessionTx`, which is `assessIsolationTx` plus the `UPDATE` onto the existing row. Pure function `computeIsolationWarnings({ restriction, machine, enforcement })`:
...
**Read** — `GET /api/v1/dialysis/sessions/:id/dialyser` returns `{ usage, device, link: { dedicated_patient_uid, baseline_tcv_ml }, reuse_restriction (projected), allowed_dispositions, isolation: { codes, required_class, warn_only, enforcement_enabled, override } }` or `{ usage: null, … }` when nothing is captured. `isolation` is not projected (§3.5). The existing `GET /sessions/:id/reuse-register` is unchanged in shape and gains the new columns.
...
| `GET /patients`, `GET /patients/:id`, `GET /today` | existing | — | responses projected per §3.5 (there is no `GET /sessions/:id`; the session read is `GET /sessions` and the today board) |
...
`startSession` and `scheduleSession` (existing routes) evaluate §3.4 and accept `isolation_override_reason`; `scheduleSession` evaluates before its insert (§3.4).
...
**Side effect the operator must plan for.** `recordMarkers` notifies the exposure handlers post-commit (`bloodborneMarkerService.js:262`), so one run of this script fires the platform handler (§3.2) **and** the cath handler for every legacy-positive patient at once: every such patient's own dedicated dialyser is quarantined (or flagged if `in_case`), every OT set used on them inside the validity window is quarantined, and one CDS alert plus one infection-control notification per patient is raised. That is the correct outcome — those devices were exposed — but it is a burst. The dry run therefore prints, beside the marker counts, the **number of devices the run would quarantine or flag** (dedicated dialysers of the affected patients not `discarded`, plus OT devices with a usage on them inside the window), and the PR body states when to run it: after the migration and the policy rows are in place, in a quiet hour, with the infection-control officer told the number first. `tested_on = updated_at::date` is any row edit (a dry-weight change moves it); under the latch it is harmless — a reactive marker's date affects only the window arithmetic for non-reactives, and a reactive row never expires.
...
- `reprocessableDeviceProjection.test.js`: roster row, serology row, restriction — for one audience role and one non-audience role each; keys blanked, never dropped. (No isolation projection exists to test, §3.5.)
...
- `dialysisReuseHookCallSites.test.js`: `cancelSession` calls `onSessionCancelledTx` before its status UPDATE and inside `setTenantTx`; `scheduleSession` calls `assessIsolationTx` before its INSERT and inside `setTenantTx`.
- `bloodborneMarkerVoidHandlers.test.js`: the void registry delivers one event per void with the row identity, a throwing handler does not stop the others, a non-function is refused.
...
- An exposure outbox (cath spec §18) and the reconciliation sweep for missed handler pushes, now across three domains; a scheduled `syncDialysisPatientSerology` sweep over the active roster — which also closes the negatives lag in §3.3.
- **Review round 1 follow-ups (recorded, not hidden):**
  ...
  - The backfill's exposure burst (§7.6): the dry run prints the affected device count; the PR body says when to run it.
  - Derived `'negative'` columns lag until the next event or the sweep (§3.3) — cosmetic.
  - A reviewed admin command to correct a wrong legacy `'positive'` (void the backfilled marker, clear the column, named actor and reason) — none exists (§3.3).

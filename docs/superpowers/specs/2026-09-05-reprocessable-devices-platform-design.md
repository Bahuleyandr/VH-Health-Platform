...
**The resolver contract this lane consumes** (agreed with dev-ea; the one addition is flagged below):
...
Decision = {
  ...
  isolation_class: 'hbsag' | 'hcv' | 'hiv' | 'isolation_mixed' | null,   // Plan 4's request - see below
...
- **`isolation_class` is this lane's one addition to the agreed four fields**, asked of dev-ea in the hand-over. The session evaluator must route a restricted patient to a machine of the *matching* class (§3.4), and the class is the *only* thing it needs beyond `status`; without it the evaluator would have to request marker detail, which the contract forbids. The class is a routing instruction, not a marker value (the same ruling review round 1 made for `required_class`, §3.5), computed by Phase 1 from the rule this lane hands over: one reactive core marker → its class; two or more → `isolation_mixed`; `restricted` for a marker with no machine class (`cjd_suspected`, other) → `isolation_mixed`; not restricted → `null`. Until dev-ea confirms the field, the adapter's stub carries it and the plan marks the machine-routing deep tests **DEPENDS ON Phase 1**.

**Snapshot versus decision** (dev-ea's fourth point). `reuse_screen`, frozen on the usage row at dialyser capture, is an **evidence snapshot**: "what was known when this dialyser was captured". It is written once, is never re-read as current state, and is never an input to any decision. Both decisions this lane makes for dialysis **re-resolve live** through the resolver: schedule / start / reassign machine routing (§3.4) and the post-use reprocess / quarantine / discard decision (§5.1). The snapshot exists so an audit can see what the capturing user saw; a marker recorded between capture and start must — and does — reach the machine assignment, and one recorded between capture and the reprocessing record reaches the disposition. `post_use_screen` is the same kind of object: the `Decision` the reprocessing record acted on, frozen after the fact. Neither snapshot carries marker detail.

**Exposure markers on a dialysis device.** `returnDeviceTx` stamps `exposure_markers` from the post-use screen's reactive markers; a `Decision` has none. For dialysis the markers are derived from `Decision.isolation_class`: `hbsag` / `hcv` / `hiv` → that one marker; `isolation_mixed` → the flag alone with `metadata.exposure_isolation_class = 'isolation_mixed'` (the CHECK `exposure_flag OR cardinality(exposure_markers) = 0` admits a flagged device with an empty list). The late-reactive handler (§3.2) is unaffected: it stamps the marker its event names. The CSSD queue reads the flag; the class is enough for universal precautions.
...
**Kept in this lane (unblocked, additive):** `dialysis_machines.isolation_class` (§4.6); `evaluateIsolationForSessionTx` / `assessIsolationTx` (warn-only, per-tenant `block`), now calling the resolver; `required_class` as a routing instruction (D10 stands); dialyser identities, the register and the 418 grain; the OT / CSSD arms; the reprocessable-devices tables; the un-capture (G1). The dialysis-arm tasks that call the resolver are marked **DEPENDS ON Phase 1 landing on `main`**: a named interface stub is acceptable for unit tests; the dialysis deep suite runs only once the resolver is on `main`, behind a named guard, never against a stub.
...
| decision `restricted`, machine registered, `isolation_class` ∉ {`decision.isolation_class`, `isolation_mixed`} | `DIALYSIS_ISOLATION_MACHINE_MISMATCH` (payload `required_class = decision.isolation_class`) | warn, or block when `enforcement = 'block'` |
...
- `reuse_restriction` on every payload: `projectReuseRestrictionForRole` (existing) — for OT the cath shape: `status`, `validity_days`, `evaluated_at` survive; `reasons` and `markers` empty. For dialysis the payload is the resolver's `Decision` (§3.3): `status`, `asOf`, `evidence`, `isolation_class` survive; `reasons` emptied; `markers` is never present on these surfaces (no caller in this lane forwards `includeMarkers`; the projection drops the key anyway, and the canary's poisoned Decision carries one so a forwarding route fails).
- `isolation` / `isolation_warnings[]`: **not projected** — `codes` and `required_class` reach every dialysis role. A required isolation class is an infection-control *routing* instruction that whoever puts the patient on a machine must know. Blanking it would protect nothing: `GET /machines` returns `isolation_class` unprojected, and the `DIALYSIS_ISOLATION_MACHINE_MISMATCH` / `DIALYSIS_GENERAL_PATIENT_ON_ISOLATION_MACHINE` pair lets any role recover the class by elimination in at most three attempts; and it would break the one workflow the feature exists for. The class names a machine, not a marker value and not the lab evidence — those stay blanked below. (Review round 1 reversed the first draft's blanking of `required_class`; the canary predicate does not treat it as a disclosure, §3.6.)
...
**Consequence the owner must see (D10).** The dialysis technician is the person who runs the reuse room and, in most units, the person who puts a patient on a machine. Under this projection the technician sees the restriction status, the isolation codes and the required machine class — enough to route the patient — but not the marker values, the reasons or the lab rows. Whether the technician (and the two blood-bank roles in the same position) joins the serology audience is D10 (§2): a one-line, reviewed widening of the pinned 35-entry allow-list in the canary, tied to the assignability question in §9.1. The design ships the same either way.

### 3.6 The canary gains three mounts

`serologyDisclosureCanary.test.js` enumerates exactly three mounts today (`/api/v1/cath-lab`, `/api/v1/stemi-pathway`, `/api/v1/cath-reprocessing`). It gains `/api/v1/dialysis` (`DIALYSIS_ROUTE_ROLES`), `/api/v1/cssd` (`CSSD_ROUTE_ROLES`) and the new `/api/v1/reprocessing` (§6.4). The stubbed persistence layer poisons the dialysis roster row with `hbsag_status = SENTINEL`, a `dialysis_serology` row with `hbsag = SENTINEL`, and both resolvers' stubs — `resolveReuseStatus` for OT and the Phase 1 `Decision` for dialysis — return sentinel-bearing `reasons`, so every unprojected `reuse_restriction` would leak. The disclosure predicate gains two checks beside the existing ones: any object carrying a non-null `hbsag_status` / `hcv_status` / `hiv_status` (or a non-null `hbsag` / `anti_hcv` / `hiv` / `hbs_titre` / `hcv_pcr`, or `is_seroconversion === true`); any `reuse_restriction` (or `restriction`) with a non-empty `reasons` or `markers`. `required_class` is deliberately **not** a disclosure (§3.5). The reachable-set snapshot is regenerated deliberately (`CANARY_WRITE_SNAPSHOT=1`, which rewrites and then fails) and the diff read: every new line is a GROWTH, which is expected for three new mounts and nothing else.
...
**Read** — `GET /api/v1/dialysis/sessions/:id/dialyser` returns `{ usage, device, link: { dedicated_patient_uid, baseline_tcv_ml }, reuse_restriction (the live Decision, projected), allowed_dispositions, isolation: { codes, required_class, warn_only, enforcement_enabled, override } }` or `{ usage: null, … }` when nothing is captured. `isolation` is not projected (§3.5); `required_class` is `Decision.isolation_class`. DEPENDS ON Phase 1. The existing `GET /sessions/:id/reuse-register` is unchanged in shape and gains the new columns.
...
| `RPD_ISOLATION_DECISION_INVALID` | 500 | the resolver answered with no `Decision` for a uid, a `status` outside the three values, or an unknown `isolation_class` — a wrong shape must not route a patient (§3.3) |
...
- `reprocessableDeviceProjection.test.js`: roster row, serology row, restriction (both shapes: the cath `reuse_restriction` and the dialysis `Decision`) — for one audience role and one non-audience role each; keys blanked, never dropped. (No isolation projection exists to test, §3.5.)
- `dialysisIsolationAdapter.test.js`: against the **named stub** (`__stubDialysisIsolationResolver`), the adapter calls the resolver with the tx client it was given, passes `includeMarkers` only when told, returns a snapshot with no `markers` key for freezing, refuses a `Decision` whose `status` is outside the three values, and throws `RPD_ISOLATION_RESOLVER_UNAVAILABLE` when the Phase 1 module is absent instead of answering.
...
Open contract point with dev-ea (not an owner decision): `isolation_class` as the fifth `Decision` field (§3.3) — the session evaluator cannot route a machine without it and may not ask for marker detail.
...
- **Carve-out follow-ups:** `isolation_class` on the `Decision` (open with dev-ea, §9); a dialysis validity window if a unit ever wants one (`serology_validity_days` is inert for dialysis today, §4.1); an amber `unknown` chip on the Staff today screen once Phase 1's roster chip exists (this lane shows only `isolation_required`).
...
- Staff refusal messages localised (`ApiResponse.code` mapping); OPEN-21 linguistic review of the 260 new non-English lines (65 keys × 4 locales).

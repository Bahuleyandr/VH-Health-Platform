...
4. **The procedure record on a cancelled case — REFUSE, BUT GIVE IT A DOOR.** Answered by the owner on 2026-09-06 and carried in the design proper as **decision 13** (§3) and **§4.8**. This was the one item that blocked implementation Task 2; it no longer does.
   ...
   **Why the other two doors were rejected** (recorded so a reader in six months finds the discarded options and their reasons, not just the survivor):

   - **A distinct `reopened` status** was priced and rejected: `cath_lab_cases_status_check` pins the seven existing statuses, so a new one costs a migration, a CHECK amendment and a new board vocabulary, and buys nothing the mandatory reason plus the `cath_lab.case_reopened` audit does not already provide. `readiness_pending` is an existing pre-start status and does the same job.
   - **`cancelled → in_progress` routed through `startCaseTx`** (proposed by the merge authority, then withdrawn by them) was rejected because it collapses two different events, reopening a case and starting it, into one action. Reopening has uses other than "start it now": the obvious one is a **mistaken cancellation** - wrong case picked off the list, cancelled in error, noticed a minute later, patient nowhere near the lab. Under that door the correction could only be made by declaring the procedure started, writing a false `actual_start_at` onto a clinical record and putting the case on the board as running. Under `cancelled → readiness_pending` the correction is just a correction, and a case that is about to run then passes Start, and its consent gate, like any other. The cost accepted is one extra action in the crash scenario; the owner's principle is that the checklist must not restrict, not that it must minimise taps, and consent fires either way.

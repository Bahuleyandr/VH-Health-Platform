...
- Date: 2026-09-06
- Status: **draft, awaiting owner design approval of the documents**. Docs only; no code on this branch. The five owner decisions below are confirmed with the merge authority (dev-1b); the lane merges on dev-1b's green verification, with no further owner gate on those five. **One question is open and blocks the start of implementation Task 2: §10.2, the procedure record on a cancelled case.**
...
**Not on this list, deliberately: what a procedure log does to a `cancelled` case.** The first draft carried it here as decision 12 ("refused by the transition table's own error"). It is **not decided** — it is the one open owner item, §10.2. `cancelled` is terminal in `CASE_TRANSITIONS`, so a refusal there is a refusal with no in-app alternative, which is the exact thing the owner's principle forbids. The other half of that draft decision is not in question and is stated where it is implemented (§4.2): with one start path, `recordProcedureLog` on `in_progress` or `completed` inserts the log and leaves the case alone — an amendment after the fact, never a re-animation.
...
**`cancelled` is the open owner item (§10.2), not a decided default.** The *proposed* behaviour is a refusal — `validateCaseTransition('cancelled', 'in_progress')` throws 400 `INVALID_STATE_TRANSITION` with `allowed: []` before anything is written, replacing today's silent force-start. It ships behind a named constant with a TODO-owner marker (`REFUSE_PROCEDURE_LOG_ON_CANCELLED`, plan Task 2 Step 5) and **is not final until the owner answers §10.2**, because a refusal here closes the only door there is. Whichever companion the owner picks, it lands in the same lane: (i) the reopen transition in Task 2, or (ii) the performed-after-cancellation record path in Task 3.
...
- **The parameterised hole is closed with a literal list.** A start path could also write `SET status = $3` and bind `'in_progress'`, which no literal-text match can see. So the same pin asserts that, among SQL literals naming `cath_lab_cases`, *any* `SET status =` assignment (parameterised or not) occurs only in a named allow-list of four `path:function` pairs — `cathLabService.js`: `startCaseTx`, `transitionCaseStatus`, `updateReadinessCheck`; `cathLabReadinessService.js`: `recomputeCaseStatusTx` — deliberately a literal list, not a prefix (the `labExternalResultCallSites.test.js` rule). A fifth writer of the case status has to be argued for in a diff.
...
| `INVALID_STATE_TRANSITION` (`AppError.invalidTransition`) | 400 | `in_progress` from `requested` / `completed` / `cancelled` / `in_progress`, as today. **Proposed, not decided:** also a procedure log on a `cancelled` case — open owner item §10.2, behind `REFUSE_PROCEDURE_LOG_ON_CANCELLED`. Companion (i) removes this row's `cancelled` clause; companion (ii) replaces it with a record path that raises `CATH_LAB_CANCELLED_RECORD_REASON_REQUIRED` (400) when its reason is empty. |
...
## 10. Owner items — decisions recorded (§10.1) and one open question (§10.2)
...
### 10.2 OPEN OWNER ITEM — the procedure record on a cancelled case

**Status: PENDING OWNER. Blocks the start of plan Task 2.**

**Today.** `recordProcedureLog` on a `cancelled` case silently force-sets `status = 'in_progress'` and `actual_start_at` (`cathLabService.js`, the `cathCase.status !== 'in_progress'` UPDATE). It looks accidental — it is the same un-guarded branch that force-starts a `scheduled` case — and the first draft of this design proposed simply removing it.

**Proposed behaviour.** `recordProcedureLog` on a `cancelled` case is **refused** (400 `INVALID_STATE_TRANSITION`, `allowed: []`) before the log row is written; `completed` and `in_progress` are left alone (the log is an amendment, the case is untouched).

**Why it cannot be a named default.** That accidental-looking path is the ONLY door by which a procedure that actually happened on a cancelled case can be recorded, because in `CASE_TRANSITIONS` **`cancelled` is terminal, so refusing here means a performed procedure on a cancelled case cannot be recorded in-app by any route; today's silent un-cancel is the only existing path.** The clinical shape is ordinary, not exotic: a case is cancelled because the list changed or the patient was unstable, the patient then crashes and is taken to the lab anyway, and the record has nowhere to land. Closing the door without opening another is a refusal with no in-app alternative — the exact thing the owner's principle (§1) forbids. That is why this is a question and not decision 12.

**If the refusal stands, the owner must choose a companion.** These are not optional follow-ups; one of them ships in this lane beside the refusal.

**(i) `cancelled` gains a transition, reachable only with a reason, routed through `startCaseTx`** — either to `in_progress`, or to a distinct `reopened` status carrying its own audit action. Consent still blocks, because the door is the same door.
- *Cost, line 1:* status vocabulary and the transition table stop saying "cancelled is the end" — `CASE_TRANSITIONS.cancelled` gains a target, every reader that treats `cancelled` as terminal has to be re-read, and `startCaseTx` must clear `actual_end_at` on the reopen or migration 482's `cath_lab_cases_actual_time_check` (`actual_end_at >= actual_start_at`) rejects the row, since cancelling stamps `actual_end_at`.
- *Cost, line 2:* the `reopened` variant additionally needs a migration to widen `cath_lab_cases_status_check` (migration 482 pins the seven statuses), a new value in every status vocabulary (day-list filters, OpenAPI enum, Staff labels), and a Staff action to reach it — this lane's "no migration" claim would end.

**(ii) A separate "performed after cancellation" record path** — writes the procedure log and an `audit_logs` row and leaves the case in `cancelled`. No status write, no `actual_start_at`, no `startCaseTx`.
- *Cost, line 1:* no status change, no migration, no vocabulary change, and the start-path pin's counts are untouched — the cheapest to build.
- *Cost, line 2:* a performed procedure then exists in two shapes (a started case with an at-start snapshot; a cancelled case with a log, no snapshot and no `actual_start_at`), so every reader gains a second shape — the day list, the monthly report's audit query, the timeline, Staff — and consent was never asserted on that path, so the audit row has to carry the consent status as it stood.

**Working proposal: (i), PENDING OWNER.** It keeps one start path, one snapshot and one consent block, which is the whole architecture of this lane; (ii) buys its cheapness by creating a second way a procedure can exist. Recommended shape of (i): the plain `cancelled → in_progress` transition (no `reopened` status, no migration), reason required, its own audit action `cath_lab.case.reopened_for_procedure_record`. **Not adopted until the owner answers.**

**Until then:** the refusal ships behind `REFUSE_PROCEDURE_LOG_ON_CANCELLED` with a TODO-owner marker (plan Task 2 Step 5), and plan Task 0 gates the start of Task 2 on this answer, carrying the concrete code for both branches so the executor implements rather than designs.
...
8. **Not a #1018 conflict, but it rides in the same hand-back: the open owner item (§10.2).** The procedure record on a `cancelled` case is **OPEN**, not decided. It gates the start of plan Task 2, it is listed as open in the PR body under "Open owner item: procedure record on a cancelled case", and the implementation PR must not be handed to the merge authority as complete while it is unanswered — the refusal that ships behind `REFUSE_PROCEDURE_LOG_ON_CANCELLED` is a placeholder, not the design.
...
| `cath_lab_cases_status_check` pins the seven statuses; `cath_lab_cases_actual_time_check` is `actual_end_at >= actual_start_at`; cancelling stamps `actual_end_at` | migration `482_cath_lab_cases_readiness.sql`; `cathLabService.js` — `transitionCaseStatus`'s `actual_end_at = CASE …` (both are costs of §10.2 companion (i)) |

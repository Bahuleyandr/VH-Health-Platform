...
REDISCLI_AUTH="$REDIS_CONTROL_PASSWORD" "$REDIS_CLI_BIN" --no-auth-warning \
  --user "$REDIS_CONTROL_USERNAME" ping | grep -qx PONG
write_fence="$(
  REDISCLI_AUTH="$REDIS_CONTROL_PASSWORD" "$REDIS_CLI_BIN" --no-auth-warning --raw \
    --user "$REDIS_CONTROL_USERNAME" \
...
role="$(REDISCLI_AUTH="$REDIS_CONTROL_PASSWORD" "$REDIS_CLI_BIN" --no-auth-warning --raw \
  --user "$REDIS_CONTROL_USERNAME" ROLE | sed -n '1p')"

if [ "$master_host" = "$local_host" ]; then
  [ "$role" = "master" ]
  replication="$(REDISCLI_AUTH="$REDIS_CONTROL_PASSWORD" "$REDIS_CLI_BIN" --no-auth-warning --raw \
    --user "$REDIS_CONTROL_USERNAME" INFO replication)"

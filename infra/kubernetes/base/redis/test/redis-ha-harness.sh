...
MASTER_0="$elected" MASTER_1="$elected" \
  DATA_PREVIOUS="$DATA_PASSWORD_PREVIOUS" TEST_HOSTNAME=redis-0 run_redis
assert_contains "$RUNTIME_DIR/redis.conf" "replicaof $elected 6379"
assert_contains "$RUNTIME_DIR/users.acl" ">$DATA_PASSWORD >$DATA_PASSWORD_PREVIOUS"
...
assert_contains "$RUNTIME_DIR/sentinel.conf" "sentinel sentinel-pass $SENTINEL_PASSWORD"
assert_contains "$RUNTIME_DIR/sentinel.conf" "sentinel monitor vhhealth-primary $sentinel_master 6379 2"
assert_contains "$RUNTIME_DIR/sentinel.conf" "sentinel auth-pass vhhealth-primary $DATA_PASSWORD"
assert_contains "$RUNTIME_DIR/sentinel-users.acl" ">$SENTINEL_PASSWORD >$SENTINEL_PASSWORD_PREVIOUS"

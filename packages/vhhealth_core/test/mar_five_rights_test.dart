...
void main() {
  ...
  test('all rights pass for a matching scan within window', () {
    final r = evaluateFiveRights(
      dose: dose(),
      scannedPatientUid: pid.toLowerCase(),
  ...
  test('drug name substring matches either direction', () {
    expect(
      evaluateFiveRights(
        dose: dose(name: 'Paracetamol 500'),
  ...
  test('dose falls back to dosage; route presence; time window 60 min', () {
    ...
    expect(
      evaluateFiveRights(
        dose: dose(sched: sched.toIso8601String()),
        scannedPatientUid: pid,
        scannedBarcode: 'Paracetamol',
        at: sched.add(const Duration(minutes: 90)),
      ).time,
      isFalse,
    );
    expect(
      evaluateFiveRights(
        ...
        scannedPatientUid: pid,
        scannedBarcode: 'Paracetamol',
        at: sched.add(const Duration(minutes: 30)),
      ).time,
      isTrue,
    );
  });

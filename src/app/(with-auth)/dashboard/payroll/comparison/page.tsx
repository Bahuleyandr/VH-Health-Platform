...
export default function PayrollComparisonPage() {
  ...
  const [fromYear, setFromYear] = useState(currentMonth > 5 ? currentYear : currentYear - 1);
  const [toMonth, setToMonth] = useState(currentMonth);
  const [toYear, setToYear] = useState(currentYear);
  const [staffFilter, setStaffFilter] = useState<string>('');
  const [viewMode, setViewMode] = useState<'all' | 'individual'>('all');

  const { data: comparisonRaw, isLoading, error } = useQuery({
    queryKey: ['payroll-comparison', fromMonth, fromYear, toMonth, toYear, staffFilter],
    queryFn: () =>
      getPayrollComparison(fromMonth, fromYear, toMonth, toYear, staffFilter || undefined),
  });
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      <h1 className="text-2xl font-bold mb-6 text-gray-900">Payroll Comparison</h1>
      ...
      {comparison && (
        <div className="grid grid-cols-2 gap-4 mb-6 sm:grid-cols-4">
          <div className="bg-blue-50 rounded-lg p-4 border border-blue-200">
            ...
            <p className="text-2xl font-bold text-blue-600">{comparison.total_staff}</p>
          </div>
          <div className="bg-green-50 rounded-lg p-4 border border-green-200">
            <p className="text-sm text-gray-600">Total Payslips</p>
            <p className="text-2xl font-bold text-green-600">{comparison.total_payslips}</p>
          </div>
          <div className="bg-purple-50 rounded-lg p-4 border border-purple-200">
            <p className="text-sm text-gray-600">Months</p>
            <p className="text-2xl font-bold text-purple-600">{comparison.month_range.length}</p>
          </div>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-12">
          <p className="text-gray-500">No payslips found for the selected period and filters.</p>
        </div>
      ) : (
        <div className="overflow-x-auto">
          {viewMode === 'all' ? (
            <ComparisonTableAll staffList={staffList} monthRange={comparison?.month_range || []} monthName={monthName} />
          ) : (
    ...
    <table className="w-full border-collapse text-sm">
      <thead>
        <tr className="border-b-2 border-gray-300 bg-gray-50">
          <th className="text-left px-4 py-3 font-semibold text-gray-700 sticky left-0 bg-gray-50 z-10 w-40">Staff Name</th>
          {monthRange.map((m) => (
            <th key={`${m.year}-${m.month}`} className="text-right px-3 py-3 font-semibold text-gray-700 whitespace-nowrap">
      ...
      <tbody>
        {staffList.map((staff) => (
          <tr key={staff.staff_uid} className="border-b border-gray-200 hover:bg-gray-50">
                ...
                >
                  {payslip ? (
                    <div className="text-sm">
                      <div className="font-semibold text-teal-700">{fmtCurrency(payslip.net_salary)}</div>
                      <div className="text-xs text-gray-500">Gross: {fmtCurrency(payslip.gross_salary)}</div>
                    </div>
                  ) : (
...
function ComparisonTableIndividual({
  ...
  const components = [
    ...
    { label: 'Gross', key: 'gross_salary', color: 'font-semibold text-gray-900' },
    ...
    { label: 'Total Deductions', key: 'total_deductions', color: 'font-semibold text-red-900' },
    { label: 'Net Pay', key: 'net_salary', color: 'font-bold text-teal-700 text-lg' },
  ];
  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
      ...
      <table className="w-full border-collapse text-sm">
        <thead>
          <tr className="border-b-2 border-gray-300 bg-gray-50">
            <th className="text-left px-4 py-3 font-semibold text-gray-700 w-32">Component</th>
            {monthRange.map((m) => (
              <th key={`${m.year}-${m.month}`} className="text-right px-4 py-3 font-semibold text-gray-700 whitespace-nowrap min-w-[130px]">
                <div>{monthName(m.month)}</div>
                <div className="text-xs text-gray-500">{m.year}</div>
              </th>
            ))}
        ...
        <tbody>
          {components.map((comp) => (
            <tr key={comp.key} className="border-b border-gray-200 hover:bg-gray-50">
              <td className={`px-4 py-3 font-medium text-gray-700 ${comp.key === 'gross_salary' || comp.key === 'net_salary' ? 'bg-gray-50 font-semibold' : ''}`}>
                {comp.label}
              </td>
              {monthRange.map((m) => {
                const payslip = staff.payslips.find((p: any) => p.month === m.month && p.year === m.year);
                ...
                return (
                  ...
                  >
                    {value !== null && value !== undefined ? fmtCurrency(value as number) : '—'}
                  </td>
                );
              })}
            </tr>
              ...
              return (
                <tr key={`att-${m.year}-${m.month}`} className="border-t border-blue-100">
                  ...
                  <td className="py-1 text-gray-600">
                    P: {payslip?.days_present || '—'} | A: {payslip?.days_absent || '—'} | L: {payslip?.lop_days || '—'} | OT: {payslip?.overtime_hours || '—'}h
                  </td>
                </tr>
              );
            })}

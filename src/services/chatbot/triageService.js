...
const PROVIDER = (process.env.CHATBOT_PROVIDER || 'anthropic').toLowerCase();
const MODEL = process.env.CHATBOT_MODEL
  || (PROVIDER === 'openai' ? 'gpt-oss-20b' : 'claude-opus-4-6');
 ...
 * @returns {Promise<object>} parsed triage JSON plus a `raw` field + `provider`
 */
export async function triageSymptoms({ symptoms, history = [], patientContext = null }) {
  ...
  return { ...parsed, raw: rawText, provider: PROVIDER };
}
